...
const root='doc/oldstreet-lan-candidate',raw=readFileSync(`${root}/correction/candidate.png`)
if(createHash('sha256').update(raw).digest('hex')!=='297a8e0a6b3e4f5f005d731c6392b2001a8476c1ec8a05045fc349008abe93d9')throw Error('SOURCE_CHANGED')
...
for(let row=0;row<4;row++){
 ...
 for(let y=t;y<b;y++)for(let x=l;x<r;x++)p.data.copy(out.data,((y+dy)*1024+row*256+x+dx)*4,(y*w+x)*4,(y*w+x)*4+4)
 frames.push({direction:['down','left','right','up'][row],sourceBox:[l,t,r,b],offset:[dx,dy],foot:[128,328]})
}
...
writeFileSync(`${root}/preparation.json`,JSON.stringify({algorithm:'reviewed-magenta-connected-matte-no-scale-1',sourceRetained:true,removed:tail,frames,width:1024,height:352,sha256:createHash('sha256').update(bytes).digest('hex'),productionAdmitted:false,limitations:['South pose has staggered feet; standing candidate only','Walking frames contain wrong directions; excluded','No generative repair performed by cutout']},null,2)+'\n')
