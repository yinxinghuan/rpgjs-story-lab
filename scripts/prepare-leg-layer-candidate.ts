...
const dir='doc/platform-art-candidates/20260913/hero-leg-layer-08'
const source=readFileSync(dir+'/candidate.png'),hash=(b:Uint8Array)=>createHash('sha256').update(b).digest('hex')
if(hash(source)!=='c8769255e3b9943c0358f4fcf5f5850feedf7d7ea43b5e11ea6cfd1159185404')throw Error('LEG_SOURCE_CHANGED')
const input=PNG.sync.read(source)
// Preserve both original cell coordinates. Only matte/edge cleanup; mirrored
// motion remains a read-only renderer experiment, never baked into this PNG.
const spec={columns:2,rows:1,cellWidth:320,cellHeight:320,foot:{x:155,y:305},sourceAnchors:[{x:155,y:305},{x:155,y:305}],kind:'states' as const,backgroundMode:'pale-neutral' as const,neutralMin:200,chromaMax:20}
