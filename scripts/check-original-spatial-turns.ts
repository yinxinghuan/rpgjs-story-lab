...
console.log(JSON.stringify({result:'pass',kind:'original-engine-spatial-turns',sourceFiles:Object.fromEntries(files.map(f=>[f,createHash('sha256').update(readFileSync(join(source,f))).digest('hex')])),results,limits:['authoring geometry; this source-turn check does not grant art admission','not an RPG-JS renderer pass','no production save import','no model requests','later chapters and finale execution still require integration']},null,2))
