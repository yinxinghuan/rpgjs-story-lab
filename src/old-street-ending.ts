...
export function completeOldStreetEnding(save:StorySave,cartridge:StoryCartridge):void{
 ...
 if(flag('roof-bridge-laid'))preserved.push(t('屋顶破损处架起了通往北侧柜子的木板。','A plank now crosses the damaged roof decking to the north cabinet.'))
 if(flag('roof-negative-returned'))preserved.push(t('你找回的底片留在照相馆，以后仍能重新冲印。','The recovered negative remains at the studio, ready for future prints.'))
 else if(flag('roof-negative-taken'))preserved.push(t('对应旧照的底片套也被你带回了家。','You also brought home the negative sleeve matching the old photograph.'))
 ...
 if(save.facts['darkroom-photo-exhibited']===true)preserved.push(t('暗房显出的旧街照片留在修表铺的公共记录册旁，你带回了它的故事。','The street photograph developed in the darkroom remains by the watch shop public record. You brought its story home.'))
 else if(save.facts['darkroom-photo-choice']==='keep')preserved.push(t('你带回了一张在暗房看清的旧街照片。','You brought home the street photograph you revealed in the darkroom.'))
 if(save.facts['darkroom-photo-choice']==='leave')preserved.push(t('看清细节的旧街照片留在暗房，等下一位来客翻看。','The completed street photograph remains in the darkroom for another visitor.'))
 if(typeof save.facts['darkroom-photo-matched']==='string'&&typeof save.facts['darkroom-photo-discovery']==='string')preserved.push(save.facts['darkroom-photo-discovery'])
 if(typeof save.facts['field-note-finding']==='string')preserved.push(save.facts['field-note-finding'])
 if(typeof save.facts['field-note-copy']==='string')preserved.push(t('你给家人带回了在照相馆抄录的便笺副本。','You brought your family the written copy you made at the studio.'))
 if(save.facts['field-note-disposition']==='take')preserved.push(t('补充便笺的原件随信带回了家。','You brought the supplementary note home with the letter.'))
 if(save.facts['field-note-disposition']==='leave')preserved.push(t('你把补充便笺留在旧街，给家人讲了里面的发现。','You left the supplementary note on the street and told your family what you found.'))
