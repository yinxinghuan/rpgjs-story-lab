...
export function composeRepairFrames(inputs:SpriteFrameSelection[]):PixelRaster{
 ...
 return composeStateFrames(inputs)
}
/** Explicit ordered state selections, bounded by the same raster budget. */
export function composeStateFrames(inputs:SpriteFrameSelection[]):PixelRaster{
 if(inputs.length<2||inputs.length>12)throw Error('SPRITE_COMPOSITION_INPUTS')
 ...
 const {width:cell,height}=sizes[0],width=cell*inputs.length
 if(sizes.some(s=>s.width!==cell||s.height!==height))throw Error('SPRITE_COMPOSITION_SIZE_MISMATCH')
