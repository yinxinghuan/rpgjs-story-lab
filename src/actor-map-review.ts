...
export function assertActorMapReview(r:any,d:{spec?:Pick<NonNullable<ActorReviewTarget['spec']>,'cellWidth'|'cellHeight'|'foot'>}):asserts r is ActorMapReview{
 const s=d.spec
...
export class ActorMapTrial{
 ...
 sample(s:RendererMotion|null,time:number){
  ...
  if(s.animation==='stand'){if(this.direction===s.direction&&this.distance>=STRIDE_DISTANCE&&poses.every(p=>this.poses.has(p))&&s.scene===home){this.stopped.add(this.direction);if(['down','left','right','up'].every(d=>this.stopped.has(d)))this.checks.add('stand');if(this.checks.has('river'))this.checks.add('return')}this.reset();return}
  if(!poses.includes(s.animation)){this.reset();return}
  // The sampler can observe the same rendered walking frame before stand arrives.
  // A duplicate earns no distance or poses, but must not erase a completed walk.
