...
export function archiveFurnitureSheets(table:string,shelf:string){
 return ['archive-index','archive-ledger','archive-desk',...Array.from({length:6},(_,i)=>`archive-storage-${i}`)].flatMap(id=>{
  if(id.startsWith('archive-storage-'))return [{id:'oldstreet-'+id,image:shelf,width:1024,height:512,framesWidth:2,framesHeight:1,textures:Object.fromEntries(['stand','hidden'].map(state=>[state,{animations:()=>[[{frameX:0,frameY:0,time:0,anchor:[.5,448/512],scale:[.14,.14],x:20,y:40,opacity:state==='stand'?1:0}]]}]))}]
