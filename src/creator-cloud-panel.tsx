...
export default function CreatorCloudPanel({repo,draft,busy,setBusy,onRestore}:{repo:ArtDraftRepository;draft?:ArtDraft;busy:boolean;setBusy:(v:boolean)=>void;onRestore:(d:ArtDraft)=>void}){
 ...
 async function operate(action:'save'|'list'|'restore',record?:CloudArtRecord){
  if(busy)return;setBusy(true);setError('');setMessage(action==='save'?t('正在保存原图…','Saving the original image…'):action==='restore'?t('正在取回并检查原图…','Retrieving and checking the original…'):t('正在读取在线草稿…','Loading online drafts…'))
  try{
   ...
   await navigator.locks.request(artDraftDatabaseName(location.href),{ifAvailable:true},async lock=>{
    ...
    if(action==='save'){
     if(!current)throw Error('ART_NOT_READY');await cloud.save(current);setRecords(await cloud.list());setMessage(t('原图已在线保存，尚未发布到游戏。','Original saved online. It has not been published to the game.'))
    }else{
 ...
 return <section aria-label={t('在线草稿','Online drafts')}><h2>{t('在线草稿','Online drafts')}</h2><p>{t('最多保存6个背景原图。使用当前浏览器的独立制作身份，尚未绑定平台账户；请保留浏览器数据以便再次取回。','Save up to six original backgrounds. Access uses this browser’s separate creation identity, not a platform account yet. Keep browser data to retrieve them again.')}</p><button disabled={busy||!draft?.candidate||!draft.taskId} onClick={()=>void operate('save')}>{t('在线保存当前候选','Save current candidate online')}</button><button disabled={busy} onClick={()=>void operate('list')}>{t('查看在线草稿','View online drafts')}</button>{message&&<p role="status">{message}</p>}{error&&<p role="alert">{error}</p>}{records?.length===0&&<p>{t('还没有在线保存的背景。','No backgrounds saved online yet.')}</p>}{records?.map((r,i)=><button key={r.id} disabled={busy||Boolean(pending)} onClick={()=>void operate('restore',r)}>{t('取回背景','Retrieve background')} {i+1} · {r.lighting==='cool'?t('清晨冷光','Cool dawn'):t('暖灯倒影','Warm lamps')} · {new Date(r.createdAt).toLocaleDateString(zh?'zh-CN':'en-US')}</button>)}</section>
}
