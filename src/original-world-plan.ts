...
export function originalBoundWorldPlan(assets?:OriginalAssetBindings,companionPositions?:Record<string,{x:number;y:number}>,sceneId?:string,approachWalkable?:(p:{x:number;y:number})=>boolean){
 ...
 world.entities=world.entities.map(e=>{
  ...
  const approach=[{x:p.x,y:p.y+28},{x:p.x,y:p.y-16},{x:p.x+20,y:p.y},{x:p.x-16,y:p.y}].find(q=>originalEnvironmentWalkable(background,e.scene,q)&&(!approachWalkable||approachWalkable(q))&&Object.entries(companionPositions??{}).every(([id,other])=>id===person?.id||!(q.x+9>other.x&&q.x<other.x+9&&q.y+15>other.y&&q.y<other.y+15)))
