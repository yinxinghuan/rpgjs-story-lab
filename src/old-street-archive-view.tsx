...
export function OldStreetArchiveView({save,readingAct,archive,target,question,locale,sessionId,api,busy,feedback,act,tryAnother,close}:{save:StorySave;readingAct:(selection:ArchiveReadingAction)=>Promise<void>;archive?:ArchiveProgress;target:string;question?:string;locale:'zh'|'en';sessionId:string;api:(path:string,body?:unknown)=>Promise<any>;busy:boolean;feedback:string;act:(type:'plan'|'observe'|'decide',order?:ArchiveCardId[])=>Promise<void>;tryAnother:()=>void;close:()=>void}){
