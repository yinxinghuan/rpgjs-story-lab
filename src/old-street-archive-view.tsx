...
export function OldStreetArchiveView({save,readingAct,archive,field,fieldAdmit,target,question,locale,sessionId,api,busy,feedback,act,tryAnother,close}:{save:StorySave;readingAct:(selection:ArchiveReadingAction)=>Promise<void>;archive?:ArchiveProgress;field?:FieldProgress;fieldAdmit:()=>Promise<void>;target:string;question?:string;locale:'zh'|'en';sessionId:string;api:(path:string,body?:unknown)=>Promise<any>;busy:boolean;feedback:string;act:(type:'plan'|'observe'|'decide',order?:ArchiveCardId[])=>Promise<void>;tryAnother:()=>void;close:()=>void}){
 ...
 return <dialog ref={root} className="os-map os-campaign" aria-labelledby="os-archive-title" onCancel={e=>{e.preventDefault();if(!busy)close()}}>
  ...
  <div className="os-campaign__body">
   ...
   {archive?.order&&target==='archive-desk'&&<OldStreetFieldLead field={field} locale={locale} id={sessionId} api={api} admit={fieldAdmit} busy={busy}/>}
  </div>
