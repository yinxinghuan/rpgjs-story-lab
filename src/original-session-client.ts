...
import {riverRejections} from './original-river-chapter'
...
export function assertOriginalClientHead(value:unknown):asserts value is OriginalHead{
 ...
 if(!h||!s||s.version!==8||s.cartridgeId!==world.cartridgeId||!['original-train-authoring-2',world.mapVersion].includes(h.mapVersion)||!room||!h.position||!originalTrainPlanWalkable(h.sceneId,h.position)||!Array.isArray(s.map)||s.map.filter(n=>n.current).length!==1||s.map.find(n=>n.current)?.id!==room.storyLocationId)throw Error('ORIGINAL_SAVE_UNSUPPORTED')
}
...
export class OriginalSessionClient extends RecoverableSessionClient<OriginalHead>{
 constructor(storage:Storage,prefix:string,transport:Transport,lock?:SessionLock){super(storage,prefix,transport,{scene:h=>h.sceneId,assertHead:assertOriginalClientHead,terminalErrors:['CHARACTER_NOT_PRESENT','ORIGINAL_NARRATION_NOT_READY','ORIGINAL_FINALE_PENDING',...riverRejections],ending:{
