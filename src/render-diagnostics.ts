...
type Node={children?:Node[];x?:number;y?:number;visible?:boolean;renderable?:boolean;alpha?:number;worldAlpha?:number;destroyed?:boolean;scale?:{x:number;y:number};worldTransform?:{tx:number;ty:number};texture?:{source?:{scaleMode?:unknown}};toWorld?:unknown;moveCorner?:unknown}
...
export function inspectDisplayTree(stage:unknown){
 ...
 function visit(n:Node,depth:number){
  ...
  if((viewport||n.texture)&&rows.length<32)rows.push({viewport,texture:Boolean(n.texture),sampling:n.texture?.source?.scaleMode==='nearest'?'nearest':n.texture?.source?.scaleMode==='linear'?'linear':null,x:number(n.x),y:number(n.y),sx:number(n.scale?.x),sy:number(n.scale?.y),wx:number(n.worldTransform?.tx),wy:number(n.worldTransform?.ty),hidden:invisible,alpha:number(n.worldAlpha??n.alpha),destroyed:n.destroyed===true})
