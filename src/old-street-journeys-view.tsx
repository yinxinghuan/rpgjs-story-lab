...
export function OldStreetJourneysView({locale,current,api,busy,select,create,close,soundEnabled,toggleSound,createCampaign}:{createCampaign?:()=>void;soundEnabled?:boolean;toggleSound?:()=>void;locale:'zh'|'en';current:string;api:(path:string)=>Promise<any>;busy:boolean;select:(id:string)=>void;create:()=>void;close:()=>void}){
 ...
 return <dialog className="os-map os-journeys" ref={dialog} aria-label={t('我的旅程','My journeys')} onCancel={e=>{e.preventDefault();dismiss()}}>
  ...
  <div className="os-journeys__body">
