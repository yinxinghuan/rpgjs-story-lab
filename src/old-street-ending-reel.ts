...
export type EndingBeat={id:string;label:string;text:string;art:'street'|'clock'|'photo'}
...
export function oldStreetEndingReel(save:StorySave):EndingBeat[]{
 ...
 return rows.flatMap((row,i)=>oldStreetDialoguePages([{id:'ending-'+i,kind:'narration',text:row.text}],save.locale).map(page=>({id:page.id,label:row.label,text:page.text,
  art:((/钟|clock/i.test(row.text)&&save.facts['clock-returned']===true)?'clock':(/照片|photograph|photo folder/i.test(row.text)&&save.facts['photos-returned']===true&&!/暗房|darkroom/i.test(row.text))?'photo':'street') as EndingBeat['art']})))
}
