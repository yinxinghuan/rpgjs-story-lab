...
export function oldStreetEnvironmentKeys(room:string,pixel:boolean,composite:boolean):string[]{
 ...
 const rooms:Record<string,string[]>={street:pixel?['streetGround','streetEdges']:[],yard:pixel?['yard','streetEdges','roofVariants']:[],shop:composite?['shopComposite']:['wood'],laundry:['laundry'],photo:pixel?['photoFloor','photoWall']:[],shed:pixel?['shedFloor','shedWall','wood']:['wood'],roof:pixel?['roofFloor','wood']:['wood'],cellar:pixel?['cellarFloor']:[],darkroom:['photoFloor']}
