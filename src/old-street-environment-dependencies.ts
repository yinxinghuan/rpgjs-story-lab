...
export function oldStreetEnvironmentKeys(room:string,pixel:boolean,composite:boolean):string[]{
 const common=['entranceShopClock','entranceShopPhoto','entranceShortPassage','entranceClockLeaf','entrancePlainLeaf','photoWall',...(room==='shed'?['shedWall']:[]),...(room==='shop'&&!composite?['shopWall']:[]),'doorWood','stoneStair','entranceFront','entranceInset','entranceGround',...(room==='street'?['entranceHome']:[]),...(pixel?['debris']:[])]
