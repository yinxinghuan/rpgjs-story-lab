...
export function oldStreetEnvironmentKeys(room:string,pixel:boolean,composite:boolean):string[]{
 ...
 const rooms:Record<string,string[]>={street:pixel?['streetGround','streetEdges']:[],yard:pixel?['yard']:[],shop:composite?['shopComposite']:['wood'],laundry:['laundry'],photo:pixel?['photoFloor','photoWall']:[],shed:pixel?['shedFloor','shedWall']:[],roof:pixel?['roofFloor']:[],cellar:pixel?['cellarFloor']:[],darkroom:['photoFloor']}
