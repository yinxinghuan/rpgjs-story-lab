...
export function oldStreetEnvironmentKeys(room:string,pixel:boolean,composite:boolean):string[]{
 ...
 const common=[...(interior?['photoWall']:[]),...(room==='shed'?['shedWall']:[]),...(room==='shop'&&!composite?['shopWall']:[]),'doorWood','stoneStair','entranceFront','entranceSide','entranceGround',...(pixel?['debris']:[])]
