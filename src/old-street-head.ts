...
export function assertOldStreetHead(value:unknown): asserts value is OldStreetHead {
  ...
  if (!h || !s || s.cartridgeId!=='old-street-letter' || s.version!==8 || !['zh','en'].includes(s.locale)
    ...
    || ![plan.mapVersion,'oldstreet-blockout-1','oldstreet-blockout-2'].includes(h.mapVersion) || !s.facts || !Array.isArray(s.map) || !Array.isArray(s.inventory)
    || !Array.isArray(s.blocks) || !Array.isArray(s.characters) || !Array.isArray(s.relationships)
    || !h.position || !oldStreetWalkable(h.sceneId,h.position,s,h.mapVersion==='oldstreet-blockout-1'?{w:9,h:15}:undefined,undefined,h.mapVersion===plan.mapVersion)) throw new LabError('OLD_STREET_SAVE_UNSUPPORTED',409)
