  ...
  return <main className={"os-dev os-dev--immersive"+(overview?" os-dev--overview":"")} data-release={OLD_STREET_PREVIEW_VERSION}>
    ...
    <div className="os-world" ref={world}><div className="os-stage" style={{width:camera.width,height:camera.height,transform:`translate(${camera.x}px,${camera.y}px)`}} ref={stage} onPointerDown={e => {
      ...
      {entities.map(e => {
        ...
        return <button className={'os-target' + (door||e.id==='street-exit' ? ' os-target--door' : '')+(renderedProps.includes(e.id)||e.id.startsWith('roof-')||e.id.startsWith('archive-')||e.id==='developing-bench'?' os-target--actor':'')} key={e.id} data-targeted={chosen?.id===e.id?'true':undefined} data-side={door?.side??(e.id==='street-exit'?'S':undefined)} data-closed={door?.gate&&!head.save.facts[door.gate]?'true':undefined} style={{left: `${e.position.x / 384 * 100}%`, top: `${e.position.y / 576 * 100}%`}}
          disabled={!ready || busy || !!outcome || !!error} onClick={() => approachObject(e)}><span className={door||e.id==='street-exit'?'os-door-label':undefined}>{title}{door?.gate && !head.save.facts[door.gate] ? text([' · 关闭', ' · closed']) : ''}</span></button>
      })}
    </div>
    </div>
