...
export default function OldStreetDev() {
  ...
  return <main className={"os-dev os-dev--immersive"+(overview?" os-dev--overview":"")} data-release={OLD_STREET_PREVIEW_VERSION}>
    ...
    <div className="os-world" ref={world}><div className="os-stage" style={{width:camera.width,height:camera.height,transform:`translate(${camera.x}px,${camera.y}px)`}} ref={stage} onPointerDown={e => {
      ...
      <div id="rpg"/>
      <svg className="os-room-foreground" viewBox="0 0 384 576" aria-hidden="true"><OldStreetDoorways room={head.scene as OldStreetRoom} facts={head.save.facts} art={environmentArt} foreground/><OldStreetRoomForeground room={head.scene as OldStreetRoom} facts={head.save.facts} art={environmentArt} actor={feet}/>{pixelShop&&<OldStreetEntranceEaves room={head.scene} image={environmentArt.streetEdges} variants={environmentArt.roofVariants}/>}</svg>
