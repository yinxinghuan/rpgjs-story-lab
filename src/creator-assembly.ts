...
import {actorReleasePath,heroReleasePath} from './actor-publication'
import {deviceReleasePath} from './device-publication'
import {layerReleasePath} from './layered-archive-contract'
export const ASSEMBLY_SLOTS=['background','actor','device','fan','hero']as const
...
export const emptyAssembly=():AssemblySelection=>({background:'',actor:'',device:'',fan:'',hero:''})
export function readAssembly(value:string|null):AssemblySelection{
 if(!value)return emptyAssembly();const r=JSON.parse(value)
 if(r&&Object.keys(r).sort().join(',')==='actor,background,device,fan')r.hero=''
 if(!r||Object.keys(r).sort().join(',')!=='actor,background,device,fan,hero'||ASSEMBLY_SLOTS.some(s=>typeof r[s]!=='string'||r[s]&&!backgroundReleaseId(r[s])))throw Error('ASSEMBLY_SELECTION_INVALID')
 return r
}
...
export async function loadAssemblyCatalog(api:Transport):Promise<AssemblyCatalog>{
 ...
 const out:AssemblyCatalog={background:[],actor:[],device:[],fan:[],hero:[]}
 await Promise.all([
  ...
  ...s.filter(d=>d.state==='ready').map(async d=>{const [a,e,h]=await Promise.all([sprites.actorPublication(d.manifest.id),sprites.publication(d.manifest.id),sprites.heroPublication(d.manifest.id)]);if(h)out.hero.push({id:h.id,name:d.manifest.draft.sourceName,createdAt:d.createdAt,preview:heroReleasePath(h.id)+'/file'});if(a)out.actor.push({id:a.id,name:d.manifest.draft.sourceName,createdAt:d.createdAt,preview:actorReleasePath(a.id)+'/file'});if(e)out.device.push({id:e.id,name:d.manifest.draft.sourceName,createdAt:d.createdAt,preview:deviceReleasePath(e.id)+'/file'})}),
