import {originalPassengerBodies} from './original-passengers'
...
export function originalWorldWalkable(head:Presence,p:Point){
 return originalCharacterWalkable(head,p,q=>originalEnvironmentWalkable(originalSceneBackgroundVersion(head.assets,head.sceneId),head.sceneId,q))&&![...originalEquipmentBodies(head.sceneId,head.assets),...originalPassengerBodies(head)].some(b=>p.x+9>b.x&&p.x<b.x+b.w&&p.y+15>b.y&&p.y<b.y+b.h)
}
...
export function originalWorldWalkabilitySnapshot(head:Presence){
 ...
 const bodies=[...originalCharacterBodies(head),...originalEquipmentBodies(scene,head.assets),...originalPassengerBodies(head)]
