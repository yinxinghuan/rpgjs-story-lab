...
const blocked=()=>Boolean(options.controlsBlocked?.())
const down=(e:KeyboardEvent)=>{if(acceptsWorldKey(e,paused||changing||blocked(),e.target instanceof Element?e.target:null)){keys.add(e.key.toLowerCase());cancelRoute();e.preventDefault()}}
...
function tick(time:number){
 ...
 const controlsBlocked=blocked()
 if(controlsBlocked&&!wasBlocked)clearInput()
 wasBlocked=controlsBlocked
 if(player&&!paused&&!changing&&!context.failed()&&!controlsBlocked){
 ...
 options.onFrame?.(dt,{...pos},activeScene,paused||changing||context.failed()||controlsBlocked||document.hidden||!player);
