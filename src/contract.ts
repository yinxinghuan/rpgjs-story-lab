...
export const entities={
 'zhou-yu':{scene:'baggage',...attendantFoot,label:['乘务员','Attendant'],states:['waiting','known'],actions:['meet-attendant','talk-attendant','check-aisle']},
 ...
 lin:{scene:'carriage',x:222,y:184,label:['修理工','Mechanic'],states:['waiting','known'],actions:['meet-lin','check-circuit','complete-handover']},
 ...
 record:{scene:'baggage',x:222,y:228,label:['调度记录','Dispatch record'],states:['unread','read'],actions:['read-record','read-arrival-code']},
 ...
 radio:{scene:'cab',x:192,y:268,label:['调度电台','Dispatch radio'],states:['unpowered','powered','routed-lights','routed-radio','acknowledged','connected'],actions:['install-battery','route-lights','route-radio','call-yard','send-signal','meet-dispatch','ask-dispatch','brief-dispatch','begin-reception','confirm-arrival','retry-arrival']},
...
export function localReply(input:string,s:StorySave,target:EntityId):Proposal {
 ...
 for(const [id,labels] of Object.entries(receptionLabels))direct[id]=labels.map(label=>label.toLowerCase())
