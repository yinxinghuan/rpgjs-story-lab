...
export const CREATOR_RUNTIME_CONTRACT='creator-background-1'
...
export type CloudArtRecord=CloudArtInput & {version:typeof CREATOR_RUNTIME_CONTRACT;bytes:number;width:1024;height:1536;createdAt:number}
...
export function assertCloudArtRecord(value:any):asserts value is CloudArtRecord{
 ...
 if(value.version!==CREATOR_RUNTIME_CONTRACT||value.width!==1024||value.height!==1536||!Number.isSafeInteger(value.bytes)||value.bytes<45||value.bytes>8*1024*1024||!Number.isSafeInteger(value.createdAt)||value.createdAt<0)throw Error('INVALID_ART_RECORD')
}
