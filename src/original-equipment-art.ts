import {yardPumpArt,originalYardPumpState} from './original-yard-pump-art'
import {dieselArt,isDieselEntity,originalDieselState} from './original-diesel-art'
...
const pump=equipmentEntities.find(e=>e.id==='yard-pump')!
...
export function originalEquipmentHasArt(id:string,assets?:OriginalAssetBindings){return id==='yard-pump'&&originalFixedEquipment(assets)['yard-pump']==='yard-pump-v1'||isDieselEntity(id)&&Boolean(originalFixedEquipment(assets)[id])||id==='brakes'&&originalFixedEquipment(assets).brakes==='brake-parts-v1'||id==='starter'||id==='tunnel-fan'&&Boolean(originalFanRelease(assets)||originalFixedEquipment(assets)['tunnel-fan'])}
export function originalEquipmentSlots(scene:string,assets?:OriginalAssetBindings){
 return [...originalYardPumpSlots(scene,assets),...originalDieselSlots(scene,assets),...(scene===brakes.scene&&originalEquipmentHasArt('brakes',assets)?['base','hose'].map(layer=>({id:'art-brake-'+layer,entityId:'brakes',layer:'brake-'+layer,graphic:brakeArt.graphic,x:brakes.position.x,y:brakes.position.y-1+(layer==='hose'?.01:0)})):[]),...(scene===starter.scene?[{id:'art-starter',entityId:starter.id,layer:'starter',graphic:starterArt.graphic,x:starter.position.x,y:starter.position.y-1}]:[]),...(scene===fan.scene&&originalEquipmentHasArt(fan.id,assets)?['housing','rotor'].map(layer=>({id:'art-fan-'+layer,entityId:fan.id,layer,graphic:'original-fan-'+layer+'-v1',x:fan.position.x,y:fan.position.y-1+(layer==='rotor'?.01:0)})):[])]
}
export function originalEquipmentAnimation(save:StorySave,layer:string,elapsed=0,assets?:OriginalAssetBindings){return layer==='yard-pump'?originalYardPumpState(save):isDieselEntity(layer)?originalDieselState(save,layer):layer==='brake-base'?'base':layer==='brake-hose'?originalBrakeState(save):layer==='housing'?'stand':layer==='rotor'?(originalFanState(save)==='running'?layerPose(elapsed,originalFanRelease(assets)?.spec.periodMs??1200):'stopped'):originalStarterState(save)}
export function originalEquipmentBodies(scene:string,assets?:OriginalAssetBindings){const g=originalStarterRelease(assets)?.review.geometry;const result=originalEquipmentSlots(scene).map(s=>g?{id:s.entityId,x:s.x-g.footprint.width/2,y:s.y+1-12,w:g.footprint.width,h:g.footprint.depth}:{id:s.entityId,x:s.x+starterArt.body.x,y:s.y+1+starterArt.body.y,w:starterArt.body.w,h:starterArt.body.h});if(scene===fan.scene&&originalEquipmentHasArt(fan.id,assets)){const body=originalFanRelease(assets)?.spec.body;result.push(body?{id:fan.id,x:fan.position.x-body.width/2,y:fan.position.y-body.depth,w:body.width,h:body.depth+body.front}:{id:fan.id,x:fan.position.x+fanArt.body.x,y:fan.position.y+fanArt.body.y,w:fanArt.body.w,h:fanArt.body.h})};if(scene===brakes.scene&&originalEquipmentHasArt('brakes',assets))result.push({id:'brakes',x:brakes.position.x+brakeArt.body.x,y:brakes.position.y+brakeArt.body.y,w:brakeArt.body.w,h:brakeArt.body.h});result.push(...originalDieselSlots(scene,assets).map(s=>({id:s.entityId,x:s.x+dieselArt.body.x,y:s.y+1+dieselArt.body.y,w:dieselArt.body.w,h:dieselArt.body.h})));result.push(...originalYardPumpSlots(scene,assets).map(s=>({id:s.entityId,x:s.x+yardPumpArt.body.x,y:s.y+1+yardPumpArt.body.y,w:yardPumpArt.body.w,h:yardPumpArt.body.h})));return result}
...
export function originalYardPumpSlots(scene:string,assets?:OriginalAssetBindings){if(scene!==pump.scene||!originalEquipmentHasArt(pump.id,assets))return [];const e=originalEntityLayout(assets,pump);return [{id:'art-yard-pump',entityId:pump.id,layer:pump.id,graphic:yardPumpArt.graphic,x:e.position.x,y:e.position.y-1}]}
