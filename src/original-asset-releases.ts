import {ORIGINAL_HERO_V2,originalHeroRelease} from './original-hero-release'
import {assertPublishedLayer,type PublishedLayer} from './layered-archive-contract'
...
const currentFixedEquipment=():FixedEquipmentBindings=>({'yard-pump':'yard-pump-v1','tunnel-fan':'tunnel-fan-parts-v1','brakes':'brake-parts-v1','river-fuel-locker':'river-diesel-v1','pine-reserve':'pine-diesel-v1','tunnel-reserve':'tunnel-diesel-v1','pass-reserve':'pass-diesel-v1','bridge-reserve':'bridge-diesel-v1'})
export const newOriginalAssetBindings=():OriginalAssetBindings=>({version:1,hero:ORIGINAL_HERO_V2,backgrounds:{...currentOriginalBackgrounds},standingCast:currentStandingCast(),fixedEquipment:currentFixedEquipment()})
const additionalBackgrounds=()=>Object.fromEntries(Object.entries(currentOriginalBackgrounds).filter(([scene])=>scene!==northCape))
function publishedBackgroundBinding(published:PublishedBackground):BackgroundBindings{const additional=additionalBackgrounds();return {version:2,hero:ORIGINAL_HERO_V2,published:structuredClone(published),standingCast:currentStandingCast(),fixedEquipment:currentFixedEquipment(),...(Object.keys(additional).length?{additional}:{})}}
...
export function originalFanRelease(bindings?:OriginalAssetBindings){return backgroundOnly(bindings)?.fan}
export function originalFixedEquipment(bindings?:OriginalAssetBindings){return backgroundOnly(bindings)?.fixedEquipment??{}}
