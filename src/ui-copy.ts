import {departureObjective} from './departure'
...
export function objectiveCopy(save:StorySave|undefined,locale:Locale):[string,string]{
 ...
 const reception=save&&(departureObjective(save,locale)??receptionObjective(save,locale));if(reception)return reception
 if(!f.repaired){
