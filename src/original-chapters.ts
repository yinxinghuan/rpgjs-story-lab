...
export const originalChapterActions=[...riverActions,...tunnelActions,...yardActions,...pineActions]
export const originalChapterBindingRules=[...riverBindingRules,...tunnelBindingRules,...yardBindingRules,...pineBindingRules]
export const originalChapterRejections=[...new Set([...riverRejections,...tunnelRejections,...yardRejections,...pineRejections])]
...
export const resolveOriginalChapter=(text:string,locale:Locale)=>resolveRiverAction(text,locale)??resolveTunnelAction(text,locale)??resolveYardAction(text,locale)??resolvePineAction(text,locale)
...
export function executeOriginalChapter(save:StorySave,c:StoryCartridge,id:ActionId){
 const river=riverActions.find(a=>a.id===id),tunnel=tunnelActions.find(a=>a.id===id),yard=yardActions.find(a=>a.id===id),pine=pineActions.find(a=>a.id===id)
 const result=river?executeRiverTurn(save,c,river.id):tunnel?executeTunnelTurn(save,c,tunnel.id):yard?executeYardTurn(save,c,yard.id):executePineTurn(save,c,pine!.id)
