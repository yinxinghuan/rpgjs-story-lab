...
import {junctionActions,junctionBindingRules,resolveJunctionAction,executeJunctionTurn,junctionChoices,junctionRejections} from './original-junction-chapter'
import {lastTrainToDawn,lastTrainToDawnEn} from './vendor/original-train/cartridges/lastTrainToDawn'
export const originalChapterActions=[...riverActions,...tunnelActions,...yardActions,...pineActions,...passActions,...townActions,...bridgeActions,...junctionActions]
export const originalChapterBindingRules=[...riverBindingRules,...tunnelBindingRules,...yardBindingRules,...pineBindingRules,...passBindingRules,...townBindingRules,...bridgeBindingRules,...junctionBindingRules]
export const originalChapterRejections=[...new Set([...riverRejections,...tunnelRejections,...yardRejections,...pineRejections,...passRejections,...townRejections,...bridgeRejections,...junctionRejections])]
...
export function projectOriginalChapterChoices(save:StorySave):StorySave{
 ...
 if(location==='dawn-junction'&&save.facts['chapter-bridge-complete'])return {...save,choices:save.finale.status==='idle'?junctionChoices(save,save.locale==='en'?lastTrainToDawnEn:lastTrainToDawn):[]}
...
export function executeOriginalChapter(save:StorySave,c:StoryCartridge,id:ActionId){
 const river=riverActions.find(a=>a.id===id),tunnel=tunnelActions.find(a=>a.id===id),yard=yardActions.find(a=>a.id===id),pine=pineActions.find(a=>a.id===id),pass=passActions.find(a=>a.id===id),town=townActions.find(a=>a.id===id),bridge=bridgeActions.find(a=>a.id===id),junction=junctionActions.find(a=>a.id===id)
 const result=river?executeRiverTurn(save,c,river.id):tunnel?executeTunnelTurn(save,c,tunnel.id):yard?executeYardTurn(save,c,yard.id):pine?executePineTurn(save,c,pine.id):pass?executePassTurn(save,c,pass.id):town?executeTownTurn(save,c,town.id):bridge?executeBridgeTurn(save,c,bridge.id):executeJunctionTurn(save,c,junction!.id)
