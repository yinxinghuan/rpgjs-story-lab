...
export function oldStreetSession(storage:Storage,lock:SessionLock,request:typeof fetch=fetch){
 ...
 const client=new RecoverableSessionClient<OldStreetHead>(storage,'oldstreet-dev-1:',api,{scene:h=>h.sceneId,assertHead:assertOldStreetHead,terminalErrors:['OLD_STREET_ACTION_UNAVAILABLE','OLD_STREET_JOURNEY_COMPLETE','OLD_STREET_INTERPRETER_NOT_READY']},lock)
 return {client,api}
}
