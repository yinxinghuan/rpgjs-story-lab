...
export function oldStreetSceneKnowledge(save:Pick<StorySave,'facts'|'locale'>,sceneId:string){
 return [...oldStreetFurniture.filter(p=>p.room===sceneId).map(p=>({id:'scenery:'+p.id,text:p.description[save.locale==='zh'?0:1]})),...oldStreetProjectedProps(save).filter(p=>p.room===sceneId).flatMap(p=>{
