...
export function actorSheet(id:string,image:string,width:number,height:number,baselines:number[][],scale=.14,centers:number[][]=[[195,181,167],[195,181,167],[195,181,167],[195,181,167]]){
 ...
 return {id,image,width,height,framesWidth:3,framesHeight:4,textures:{stand:{animations:({direction}:{direction:Direction})=>[[frame(rowFor(direction),1,0)]]},walk:{animations:({direction}:{direction:Direction})=>[[frame(rowFor(direction),0,0),frame(rowFor(direction),1,10),frame(rowFor(direction),2,20),{time:30}]]}}}
}
