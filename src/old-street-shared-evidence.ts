...
export type EvidenceKind='photo-shown'|'photo-described'|'archive-account'
...
export function sharedEvidence(save:StorySave,entity:string):SharedEvidence[]{
 ...
 return save.blocks.filter(b=>b.data?.evidenceRecipient===person.id&&typeof b.data.evidenceText==='string'&&['photo-shown','photo-described','archive-account'].includes(String(b.data.evidenceKind)))
...
export function sharedEvidenceKnowledge(save:StorySave,entity:string){
 ...
 return sharedEvidence(save,entity).filter(e=>!shown||e.kind!=='photo-described').map(e=>({id:'received:'+e.kind,text:
  (e.kind==='photo-shown'?(zh?'玩家曾在你面前出示拼好的照片，你能看见：':'The player showed you the completed photograph. You could see: '):e.kind==='photo-described'?(zh?'玩家曾向你转述照片里的发现；你没有看过照片原图：':'The player described a finding in a photograph; you have not seen the print: '):(zh?'玩家曾告诉你以下档案调查结果；这是对方查明后分享的内容，不代表你亲历事件：':'The player shared this reconstructed archive account with you; it does not mean you witnessed the events: '))+e.text}))
}
export function evidenceRecallTopics(save:StorySave,entity:string){
 const zh=save.locale==='zh',entries=sharedEvidence(save,entity),shown=entries.find(e=>e.kind==='photo-shown'),described=entries.find(e=>e.kind==='photo-described'),account=entries.find(e=>e.kind==='archive-account'),photo=shown??described
 return [
