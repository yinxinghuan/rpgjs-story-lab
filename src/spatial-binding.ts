...
export interface SpatialBindingDefinition {
 ...
 scenes:Array<{id:string;spawn:SpatialPoint}>;
...
export function compileSpatialBinding(story:SpatialStoryDefinition,input:SpatialBindingDefinition,isWalkable:(scene:string,p:SpatialPoint)=>boolean){
 ...
 for(const id of storyScenes.keys())if(!scenes.has(id))fail('UNBOUND_STORY_SCENE',id)
 for(const [id,s] of scenes){if(!storyScenes.has(id))fail('UNKNOWN_STORY_SCENE',id);if(!checkPoint(id,s.spawn))fail('INVALID_SPATIAL_SPAWN',id)}
 ...
 for(const [id,rule] of rules){
  ...
  if(maps.length>1||maps.length!==Number(Boolean(portal))||(portal&&maps[0].nodeId!==portal.scene))fail('PORTAL_RULE_MISMATCH',id)
 }
 ...
 const current=(snapshot:SpatialSnapshot)=>{
  ...
  if(snapshot.map.some(n=>!scenes.has(n.id)))fail('UNREPRESENTABLE_SCENE','unknown saved scene')
 ...
 return {
  ...
  assertTransition:(before:SpatialSnapshot,after:SpatialSnapshot,actionId:string|null)=>{
   const from=current(before),to=current(after),portal=actionId?portals.get(actionId):undefined
   if(actionId&&(!actions.has(actionId)||entities.get(actions.get(actionId)!)!.scene!==from))fail('PORTAL_SOURCE_MISMATCH',actionId)
   if(from!==to&&(!portal||portal.scene!==to))fail('UNADMITTED_STORY_TRANSITION',from+' -> '+to)
   if(portal&&portal.scene!==to)fail('PORTAL_STORY_MISMATCH',actionId!)
