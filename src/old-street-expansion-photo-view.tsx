...
export function OldStreetExpansionPhotoView({locale,sessionId,api,disabled,matched,discovery,submit,onOpenChange,onClose,interrupted=false,choice,decide,allowRegenerate=false,requestOpen=0,nearby=true,photoMethod}:{interrupted?:boolean;onClose?:()=>void;photoMethod?:'develop-v1';locale:'zh'|'en';sessionId:string;api:(path:string,body?:unknown)=>Promise<any>;disabled:boolean;matched:boolean;discovery?:string;choice:string;allowRegenerate?:boolean;requestOpen?:number;nearby?:boolean;decide:(choice:'keep'|'leave')=>Promise<void>;submit:(proof:unknown)=>Promise<void>;onOpenChange:(open:boolean)=>void}){
