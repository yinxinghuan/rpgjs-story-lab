...
export function OldStreetExpansionPhotoView({locale,sessionId,api,disabled,matched,discovery,submit,pause,choice,decide,allowRegenerate=false,requestOpen=0,nearby=true}:{locale:'zh'|'en';sessionId:string;api:(path:string,body?:unknown)=>Promise<any>;disabled:boolean;matched:boolean;discovery?:string;choice:string;allowRegenerate?:boolean;requestOpen?:number;nearby?:boolean;decide:(choice:'keep'|'leave')=>Promise<void>;submit:(proof:unknown)=>Promise<void>;pause:(open:boolean)=>void}){
 ...
 return <div className="os-expansion">
  ...
  {job?.state==='candidate'&&nearby&&<button disabled={disabled||sending||!image||imageFailed} onClick={()=>{setFeedback('');setOpen(true);pause(true)}}>{t(matched?'查看照片':'拼合照片',matched?'View photograph':'Match photograph')}</button>}
