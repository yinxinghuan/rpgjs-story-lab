...
export function OldStreetExpansionPhotoView({locale,sessionId,api,disabled,matched,discovery,submit,onOpenChange,onClose,exhibited=false,interrupted=false,choice,decide,allowRegenerate=false,requestOpen=0,nearby=true,photoMethod}:{exhibited?:boolean;interrupted?:boolean;onClose?:()=>void;photoMethod?:'develop-v1';locale:'zh'|'en';sessionId:string;api:(path:string,body?:unknown)=>Promise<any>;disabled:boolean;matched:boolean;discovery?:string;choice:string;allowRegenerate?:boolean;requestOpen?:number;nearby?:boolean;decide:(choice:'keep'|'leave')=>Promise<void>;submit:(proof:unknown)=>Promise<void>;onOpenChange:(open:boolean)=>void}){
 const [image,setImage]=useState('')
 const [open,setOpen]=useState(false),[feedback,setFeedback]=useState('')
 const reportOpen=useRef(onOpenChange);reportOpen.current=onOpenChange
 useEffect(()=>{reportOpen.current(open);return()=>reportOpen.current(false)},[open])
 useEffect(()=>{if(interrupted)setOpen(false)},[interrupted])
 ...
 useEffect(()=>{if(requestOpen&&image&&!imageFailed&&job?.asset&&!matched){setFeedback('');setOpen(true)}},[requestOpen,image])
 ...
 return <div className="os-expansion">
  <p role="status">{exhibited?t('照片留在修表铺的公共记录册旁。','The photograph is by the public record in the watch shop.'):choice==='keep'?t('旧街照片已收进随身行囊。','The photograph is in your bag.'):choice==='leave'?t('照片留在显影台上。','The photograph remains on the developing bench.'):matched?t('旧街照片已看清，已记入这次旅程。','The clear photograph is saved in this journey.'):(failed||imageFailed)?t('照片暂时取不到，可以继续探索。','The photograph is unavailable. You can keep exploring.'):job?.state==='candidate'?(photoMethod?t('底片已备好，可以调焦与曝光。','The negative is ready for focus and exposure.'):t('照片显出来了。','The photograph has developed.')):job?.state==='preparing'?t('照片正在显影，可以先去别处看看。','The photograph is developing. You can explore elsewhere.'):job?.state==='failed'?t('显影暂时中断了。','Developing was interrupted.'):t('显影台上留着一张待冲洗的照片。','An undeveloped photograph rests on the workbench.')}</p>
  {job?.state==='candidate'&&!image&&!imageFailed&&<p role="status">{t('照片正在载入。可以先继续探索，再回显影台查看。','Loading the photograph. You can keep exploring and return to this bench.')}</p>}
