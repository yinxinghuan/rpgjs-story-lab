...
export function OldStreetExpansionPhotoView({locale,sessionId,api,disabled,matched,submit,pause,choice,decide,allowRegenerate=false}:{locale:'zh'|'en';sessionId:string;api:(path:string,body?:unknown)=>Promise<any>;disabled:boolean;matched:boolean;choice:string;allowRegenerate?:boolean;decide:(choice:'keep'|'leave')=>Promise<void>;submit:(proof:unknown)=>Promise<void>;pause:(open:boolean)=>void}){
 ...
 },[api,sessionId,job?.asset?.sha256,job?.state])
 ...
 return <div className="os-expansion">
  ...
  {job?.state==='candidate'&&!matched&&<button disabled={disabled||sending||!image} onClick={()=>{setFeedback('');setOpen(true);pause(true)}}>{t('拼合照片','Match photograph')}</button>}
