...
export function oldStreetDrawerSheet(image:string,pixel=false){
 const pose=(frameX:number)=>{const f=oldStreetPixelDrawerFrames[frameX];return {animations:()=>[[{frameX,frameY:0,time:0,anchor:[(pixel?f.center:frameX===0?243:252)/512,(pixel?f.foot:494)/768],scale:[pixel ? .16*253/f.width : .16,.16],x:16,y:28}]]}}
