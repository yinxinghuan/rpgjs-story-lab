...
function semanticActionCore(value: string, locale: StoryCartridge['locale']): string {
  ...
  return value.toLocaleLowerCase().replace(/[^a-z\s]/g, ' ').split(/\s+/).filter(Boolean)
    .filter((word) => !semanticStopWords.has(word)).join('')
}
...
/** A system tray may not manufacture a retry by swapping inspect/touch/check
 * verbs around the same object after a turn committed no authoritative progress. */
