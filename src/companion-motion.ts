...
export class CompanionMotion {
 ...
 update(dt:number,leader:CompanionPoint,options:{paused:boolean;talkingTo?:string;workPosts?:Record<string,CompanionPoint>;walkable:(p:CompanionPoint)=>boolean;findPath:(a:CompanionPoint,b:CompanionPoint)=>CompanionPoint[]}):CompanionState[]{
  ...
  for(const [index,w] of this.walkers.entries()){
   ...
   const post=options.workPosts?.[w.id],goal=post??this.target(this.spacing*(index+1))
   if(distance(w.position,goal)<3||!post&&distance(w.position,leader)<this.spacing*.8){w.pose='stand';w.route=[];w.repath=0;continue}
   w.repath-=dt
