...
export function OldStreetPhotoView({locale,busy,submit,close,feedback,photograph}:{locale:'zh'|'en';busy:boolean;feedback:string;submit:(proof:unknown)=>void;close:()=>void;photograph?:{image:string;version:string}}){
 ...
 return <dialog ref={dialog} className="os-map os-photo" aria-label={t('照片比对','Compare photographs')} onCancel={e=>{e.preventDefault();dismiss()}}>
  <header><h2>{t('接上这张旧照','Complete the old photograph')}</h2><button disabled={busy} onClick={dismiss}>{t('收起','Close')}</button></header><div className="os-photo__content"><p>{photograph?t('沿着路面和屋檐，找到能接上的另一半。','Match the road and roof lines across the two halves.'):t('找到能接上窗沿和晾衣绳的另一半。','Find the half whose window sill and clothesline continue the picture.')}</p>
  ...
  {!failed&&<div className="os-photo__controls">
