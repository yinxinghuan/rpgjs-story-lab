...
export function OldStreetExpansionView({locale,sessionId,requested,disabled,api,submit,activate,archiveTitle,commission=false,sourcePending=false,sourceRecovered=false}:{locale:'zh'|'en';sessionId:string;requested:boolean;disabled:boolean;api:(path:string,body?:unknown)=>Promise<any>;submit:(text:string,followArchive?:boolean)=>Promise<void>;archiveTitle?:string;commission?:boolean;sourcePending?:boolean;sourceRecovered?:boolean;activate:()=>Promise<void>}){
 ...
 async function start(followArchive=false){
  if(sending||disabled||sourcePending)return
 ...
 return <details className="os-expansion" open={commission}><summary>{commission?t('寻找记录中的旧照','Find a photograph of the recorded history'):t('探查暗房','Explore the darkroom')}</summary>
  {!requested&&sourcePending&&<p>{t('归档单指向屋顶北侧柜子的双缺口底片套。从旁边楼梯上去，取回后再来冲印。','The index points to a double-notched sleeve in the north roof cabinet. Take the stairs beside the studio and bring the film back for printing.')}</p>}
  {!requested&&archiveTitle&&!sourcePending&&<><p>{t('沿着档案线索寻找：','Follow the archive lead: ')}{archiveTitle}</p><button disabled={disabled||sending} onClick={()=>void start(true)}>{sourceRecovered?t('用取回的底片冲印','Print from the recovered negative'):t('寻找相关旧照','Look for a related photograph')}</button></>}
