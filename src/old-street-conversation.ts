...
export function oldStreetTalkTopics(save:StorySave,entity:string){
 ...
 const route=entity==='watchmaker'
  ...
  ?topic('steps',f['crates-cleared']?'院里的旧箱已经搬开了。':'院里的台阶怎么过去？',f['crates-cleared']?'I moved the courtyard crates.':'How do I reach the courtyard steps?',f['crates-cleared']?'谢谢，台阶终于能走了。推车用完放回原处就好。':f['trolley-borrowed']?'你已经借了推车，把台阶前的旧箱搬开就行。':'旁边有推车可以借，把挡路的旧箱搬到墙边就能过去。',f['crates-cleared']?'Thank you, the steps are clear at last. Put the trolley back when you are done.':f['trolley-borrowed']?'Use the trolley you borrowed to move the crates.':'Borrow the trolley beside me and move the crates to the wall.')
 ...
 const personal=entity==='watchmaker'
  ...
  :entity==='laundry-owner'
  ?topic('clock',f['clock-taken']||f['clock-returned']?'能说说那口旧钟吗？':'店里有什么需要帮忙的？',f['clock-taken']||f['clock-returned']?'Could you tell me about the old clock?':'Is there anything I can help with here?',f['clock-returned']?'那是母亲留下的钟。谢谢你送回来。要记录它的来历，可以先问我。':'那口钟送去修了，我还在等它回来。',f['clock-returned']?'It was my mother’s clock. Thank you for returning it. Ask me before recording its history.':'It is away for repairs. I am waiting for it to come back.')
  :topic('photos',f['photos-returned']?'这些照片你打算怎么保存？':f['photos-taken']?'你在找这些照片吗？':'有什么需要帮忙的吗？',f['photos-returned']?'How will you preserve these photographs?':f['photos-taken']?'Are these the photographs you are looking for?':'Is there anything I can help with?',f['photos-returned']?'你送回来的照片已经收好了。要留下其中一张，可以让我来选。':'我在找一份旧照片夹。上面有照相馆的标记，找到后可以拿到放大台上比对。',f['photos-returned']?'Your returned photographs are safely stored. Let me choose one if you want to share it.':'An old folder marked with this studio’s stamp. Bring it to the viewing table to compare the photographs.')
