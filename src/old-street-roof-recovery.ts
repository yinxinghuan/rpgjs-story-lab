...
export function roofRecoveryRules(locale:Locale):DomainActionRule[]{
 ...
 return [
  ...
  rule('open-roof-box',[need('roof-index-read',true,'先找到能辨认底片的归档标记。先查阅与它相关的记录。','You need a filing mark to identify the negative. Examine the associated records first.'),need('roof-bridge-laid',true,'破损处还没有通路。','There is no crossing over the damaged decking yet.'),once('roof-box-open')],[flag('roof-box-open')],'你打开柜门，按归档标记找到双缺口纸套。里面保存着对应的旧街底片。','You open the cabinet and find the double-notched sleeve described by the filing mark. It holds the matching street negative.'),
...
export function assertRoofRecovery(save:StorySave){
 ...
 if(f['roof-recovery']!==true){if(f['roof-index-origin']!==undefined||Object.keys(roofRecoveryInitial).some(k=>k!=='roof-recovery'&&f[k]!==undefined)||has.length||f['roof-plank-source']!==undefined||f['roof-plank-taken']!==undefined||save.inventory.some(i=>i.id==='roof-plank'))throw Error('ROOF_RECOVERY_UNSUPPORTED');return}
 if(f['roof-index-origin']!==undefined&&f['roof-index-origin']!=='archive')throw Error('ROOF_INDEX_ORIGIN_INVALID')
 for(const k of Object.keys(roofRecoveryInitial))if(typeof f[k]!=='boolean')throw Error('ROOF_RECOVERY_INVALID')
 if(f['roof-photo-ready']!==!!f['darkroom-photo-matched']||f['roof-index-read']&&!(f['roof-index-origin']==='archive'?f['archive-reconstructed']:f['roof-photo-ready'])||f['roof-box-open']&&(!f['roof-index-read']||!f['roof-bridge-laid'])||f['roof-negative-taken']&&!f['roof-box-open']||f['roof-negative-returned']&&!f['roof-negative-taken'])throw Error('ROOF_RECOVERY_SEQUENCE_INVALID')
