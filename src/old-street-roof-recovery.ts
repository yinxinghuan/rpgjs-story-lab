...
export const roofRecoveryActionRooms:Record<string,'darkroom'|'roof'|'photo'>={'read-photo-index':'darkroom','try-short-plank':'roof','lay-roof-plank':'roof','open-roof-box':'roof','take-roof-negative':'roof','return-roof-negative':'photo'}
export const roofRecoveryProps=[
 {id:'roof-planks',room:'roof' as const,position:{x:248,y:412},approach:{x:212,y:400},body:{x:232,y:364,w:84,h:60},actions:['oldstreet:try-short-plank','oldstreet:lay-roof-plank']},
...
export function roofRecoveryRules(locale:Locale):DomainActionRule[]{
 ...
 return [
  ...
  rule('lay-roof-plank',[once('roof-bridge-laid')],[flag('roof-bridge-laid')],'你把长板搭在两侧完整的屋面上。现在能走到北边的柜子旁了。','You rest the long plank on intact decking at both ends. You can now walk to the cabinet on the north side.'),
...
export function assertRoofRecovery(save:StorySave){
 ...
 if(f['roof-recovery']!==true){if(Object.keys(roofRecoveryInitial).some(k=>k!=='roof-recovery'&&f[k]!==undefined)||has.length)throw Error('ROOF_RECOVERY_UNSUPPORTED');return}
