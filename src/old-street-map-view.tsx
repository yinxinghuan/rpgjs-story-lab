...
export function OldStreetMapView({save,room,locale,onClose,onTravel,busy=false,error=''}:{onTravel?:(room:OldStreetRoom)=>void;busy?:boolean;error?:string;save:StorySave;room:OldStreetRoom;locale:Locale;onClose:()=>void}){
 ...
 const g=useMapGesture({width:worldWidth,height:worldHeight})
 const locate=()=>{const [x,y]=OLD_STREET_ATLAS[room];g.locate(x/atlas.width,y/atlas.height,1)}
 ...
 return <dialog className="os-map os-neighbourhood os-atlas" ref={dialog} aria-labelledby="os-map-title" onCancel={e=>{e.preventDefault();close()}}>
  ...
  <div className="os-atlas__viewport" ref={g.viewport} {...g.handlers}>
   <div className="os-atlas__world" ref={g.layer} style={{width:worldWidth,height:worldHeight}}>
  ...
  <section className="os-neighbourhood__directions" aria-label={t('路线指引','Directions')}>
   <button className="os-map-travel" disabled={!onTravel||busy||destination===room||!route||Boolean(save.facts.departed)} onClick={()=>onTravel?.(destination)}>{busy?t('正在前往…','Travelling…'):destination===room?t('你在这里','You are here'):!route?t('没有已开放的路线','No open route'):t('前往这里','Go here')}</button>{error&&<p role="alert">{error}</p>}
