...
const basePositions:Record<OldStreetRoom,[number,number]>={archive:[60,50],darkroom:[300,250],cellar:[60,50],shed:[180,50],roof:[300,50],laundry:[60,150],yard:[180,150],photo:[300,150],shop:[60,250],street:[180,250]}
export function OldStreetMapView({save,room,locale,onClose}:{save:StorySave;room:OldStreetRoom;locale:Locale;onClose:()=>void}){
 ...
 const expanded=map.rooms.some(r=>r.id==='archive'),height=expanded?400:300
 const positions=Object.fromEntries(Object.entries(basePositions).map(([id,[x,y]])=>[id,[x,y+(expanded&&id!=='archive'?100:0)]])) as Record<OldStreetRoom,[number,number]>
