...
export default function OriginalScenePreview(){
 const query=new URLSearchParams(location.search),source=query.get('art_source'),platform=source==='platform',draftMode=source==='draft',actorDraftId=query.get('actor_draft')
 ...
 useEffect(()=>{
  mounted.current=true;let draftBlob='',actorBlob='';let probe:ReturnType<typeof setInterval>|undefined
  ...
  void(async()=>{try{
   ...
   createRpgRenderer({host:document.getElementById('rpg')!,width:384,height:576,sceneIds:Object.keys(config.resources.scenes),initialScene:config.initialScene,initialPosition:spawn,heroGraphic,spritesheets:[sheet],mapEvents:()=>[],walkable:(p,s)=>originalTrainPlanWalkable(s,p),safePosition:(p,s)=>originalTrainPlanWalkable(s,p)?p:world.scenes.find(room=>room.id===s)!.spawn,findPath:(a,b,s)=>findGridPath(a,b,p=>originalTrainPlanWalkable(s,p)),onPosition:setPosition,onDestination:setDestination,onReady:r=>{if(!mounted.current){r.destroy();return}runtime.current=r;setEngineReady(true);probe=setInterval(()=>setRendered(r.renderedPosition()),150);void enterScene(config.initialScene)}})
  }catch{if(mounted.current){setSwitching(false);setError(actorDraftId?t('人物候选或场景未通过检查，请返回准备页核对。','The actor candidate or scene failed validation. Check the preparation page.'):t('初始场景未通过检查，请重新载入。','Initial scene validation failed. Reload to retry.'))}}})()
  return()=>{mounted.current=false;if(draftBlob)URL.revokeObjectURL(draftBlob);if(actorBlob){void Assets.unload(actorBlob).catch(()=>{});URL.revokeObjectURL(actorBlob);}if(probe)clearInterval(probe);runtime.current?.destroy();for(const id of Object.keys(config.resources.scenes)){const url=loader.background(id);if(url)URL.revokeObjectURL(url)}}
 },[])
 ...
 return <main className="cl-app cl-original-preview">
  ...
  <footer className="cl-original-checks" ref={checks}><nav className="cl-art-comparison" aria-label={t('背景对照','Background comparison')}><a href={`./creator.html?scene_preview=north-cape${actorSuffix}`} aria-current={!platform&&!draftMode?'page':undefined}>{t('基准背景','Baseline')}</a><a href={`./creator.html?scene_preview=north-cape&art_source=platform${actorSuffix}`} aria-current={platform?'page':undefined}>{t('平台背景候选','Platform candidate')}</a><a href="./creator.html?create_art=north-cape">{t('制作背景','Create background')}</a></nav>{actorDraftId&&<nav className="cl-art-comparison" aria-label={t('人物候选对照','Actor comparison')}><a href={`./creator.html?scene_preview=north-cape${backgroundSuffix}`}>{t('退回基准人物','Use baseline actor')}</a><a href="./creator.html?create_art=sprite">{t('返回人物准备','Back to sprite preparation')}</a></nav>}<label>{t('检查场景','Inspect scene')}<select aria-label={t('检查场景','Inspect scene')} disabled={!engineReady||switching} value={requested} onChange={e=>void enterScene(e.target.value)}>{Object.keys(names).map(id=><option key={id} value={id}>{names[id]}</option>)}</select></label><p>{notice||t('点地面行走 · 人物与剧情尚待接入。','Click to walk · Cast and story are not connected yet.')}</p><div>
