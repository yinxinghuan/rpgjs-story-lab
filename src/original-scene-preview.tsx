...
export default function OriginalScenePreview(){
 ...
 const points:Array<{label:string;position:RendererPoint}>=at?[{label:t('走到柜前','Walk in front'),position:{x:at.x-4.5,y:at.y+2}},{label:t('走到柜后','Walk behind'),position:{x:at.x-4.5,y:at.y-(device.current?.footprint.depth??12)-20}},{label:t('检查柜体阻挡','Check cabinet collision'),position:{x:at.x-4.5,y:at.y-12}}]:river?[{label:t('桥头观察位','Bridge approach'),position:{x:188,y:330}},{label:t('左侧岸边','Left bank'),position:{x:80,y:390}},{label:t('右侧岸边','Right bank'),position:{x:290,y:390}},{label:t('返回停靠方向','Train approach'),position:spawn}]:[['starter','左侧检修位','Starter side'],['brakes','右侧制动位','Brake side'],['fuel-shed','燃料棚前','Fuel frontage'],['departure-control','出站控制位','Departure position']].map(([id,zh,en])=>({label:t(zh,en),position:world.entities.find(e=>e.id===id)!.approach}))
 return <main className="cl-app cl-original-preview" data-device-state={deviceDraftId?deviceState:undefined} data-device-draft={deviceDraftId??undefined}>
  ...
  <footer className="cl-original-checks" ref={checks}>
   ...
   {deviceDraftId&&<>
    ...
    <div aria-label={t('设备状态','Device state')}>{DEVICE_STATES.map((state,i)=><button key={state} aria-pressed={deviceState===state} disabled={!ready||switching} onClick={()=>changeDevice(state)}>{[t('关柜','Closed'),t('开柜有物','Open, stocked'),t('开柜空置','Open, empty')][i]}</button>)}</div>
   </>}
