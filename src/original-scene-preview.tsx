...
export default function OriginalScenePreview(){
 ...
 return <main className="cl-app cl-original-preview">
  ...
  <footer className="cl-original-checks" ref={checks}><nav className="cl-art-comparison" aria-label={t('背景对照','Background comparison')}><a href="./creator.html?scene_preview=north-cape" aria-current={!platform&&!draftMode?'page':undefined}>{t('基准背景','Baseline')}</a><a href="./creator.html?scene_preview=north-cape&art_source=platform" aria-current={platform?'page':undefined}>{t('平台背景候选','Platform candidate')}</a><a href="./creator.html?create_art=north-cape">{t('制作背景','Create background')}</a></nav><label>{t('检查场景','Inspect scene')}<select aria-label={t('检查场景','Inspect scene')} disabled={!engineReady||switching} value={requested} onChange={e=>void enterScene(e.target.value)}>{Object.keys(names).map(id=><option key={id} value={id}>{names[id]}</option>)}</select></label><p>{notice||t('点地面行走 · 人物与剧情尚待接入。','Click to walk · Cast and story are not connected yet.')}</p><div>
  ...
  {error&&<aside className="cl-error" role="alert"><p>{error}</p>{draftMode&&<a href="./creator.html?create_art=north-cape">{t('返回制作页选择候选','Choose a candidate in the creator')}</a>}<button onClick={()=>engineReady?void enterScene(requested):location.reload()}>{engineReady?t('重试','Retry'):t('重新载入','Reload')}</button></aside>}
 </main>
}
