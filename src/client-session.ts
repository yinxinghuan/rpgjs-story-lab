...
export async function api(path:string,body?:unknown){if(isBrowserEdition){browserApi??=import('./browser-journey').then(({BrowserJourney})=>new BrowserJourney('alteru:'+GAME_ID+':pages-journeys-v1'));return (await browserApi).api(path,body)}const r=await fetch(prefix+path,{method:body?'POST':'GET',headers:{'Content-Type':'application/json',Authorization:'Bearer '+token()},body:body?JSON.stringify(body):undefined});const data=await r.json();if(!r.ok)throw new Error(data.error??'NETWORK_ERROR');return data}
