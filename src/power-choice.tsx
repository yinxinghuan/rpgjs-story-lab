...
export function radioDescription(s:StorySave,locale:Locale){const f=s.facts;return tr(locale,
...
export function radioStatus(s:StorySave,locale:Locale){const f=s.facts;return tr(locale,f.rescue_sent?'接应已定位':f.signal_acknowledged?'短报文已收 · 待引导':f.power_chosen?(f.power_radio?'电台增幅 · 客厢应急光':'客厢常亮 · 电台短报文'):f.battery_installed?'已通电 · 待选择供电':'电池槽为空',f.rescue_sent?'Rescue guidance established':f.signal_acknowledged?'Acknowledged · Guide light needed':f.power_chosen?(f.power_radio?'Radio amplified · Emergency light':'Carriage lit · Short-message radio'):f.battery_installed?'Powered · Choose a route':'Battery bay empty')}
