...
const prop = (id: string, room: OldStreetRoom, x: number, y: number, actions: string[]): Prop => {
  ...
  return {id, room, position, approach: {x: position.x, y: position.y + (id==='watchmaker'?44:28)}, actions: actions.map(oldStreetActionId), body: {x: position.x - 12, y: position.y - 12, w: 32, h: 28}}
}
