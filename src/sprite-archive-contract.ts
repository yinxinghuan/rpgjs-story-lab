...
export function assertSpriteManifest(v:any):asserts v is SpriteArchiveManifest{
 ...
 if(d.composition){const c=d.composition;if(d.deviceStateSet!=='repair'||d.sourceKind!=='states'||Object.keys(c).sort().join(',')!=='inputs,version'||c.version!==1||!Array.isArray(c.inputs)||c.inputs.length!==2||roles.size!==4)return fail();for(let n=0;n<2;n++){const i=c.inputs[n],f=v.files.find((f:SpriteFile)=>f.role==='input-'+n);if(!i||Object.keys(i).filter(k=>k!=='generation').sort().join(',')!=='column,columns,source,sourceName'||i.source!=='input-'+n||!f||typeof i.sourceName!=='string'||i.sourceName.length>100||!integer(i.columns,1,12)||!integer(i.column,0,i.columns-1)||f.width%i.columns||f.width/i.columns!==source.width/2||f.height!==source.height)return fail();if(i.generation!==undefined)assertSpriteGenerationSource(i.generation)}}else if(d.actorPatch){
