...
export function assertSpriteManifest(v:any):asserts v is SpriteArchiveManifest{
 ...
 if(s.matteSeeds!==undefined&&(!Array.isArray(s.matteSeeds)||s.matteSeeds.length>64||s.backgroundMode!=='pale-neutral'||s.matteSeeds.some((p:any)=>!p||Object.keys(p).sort().join(',')!=='x,y'||!integer(p.x,0,source.width-1)||!integer(p.y,0,source.height-1))))return fail()
