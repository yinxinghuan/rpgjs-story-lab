import {assertProtagonistIdentity} from './protagonist-identity'
...
export const SPRITE_FILE_ROLES=['source','candidate','input-0','input-1','input-2'] as const
...
export function spriteManifest(d:SpriteDraft){
 ...
 const draft={version:d.version,id:d.id,revision:d.revision,createdAt:d.createdAt,...(d.parentId?{parentId:d.parentId}:{}),source:file('source',d.source),sourceName:d.sourceName,sourceKind:d.sourceKind??d.spec.kind,...(d.actorPatch?{actorPatch:{...structuredClone(d.actorPatch),base:{...d.actorPatch.base,source:file('input-0',d.actorPatch.base.source)},frame:{...d.actorPatch.frame,source:file('input-1',d.actorPatch.frame.source)}}}:{}),...(d.deviceStateSet?{deviceStateSet:d.deviceStateSet}:{}),spec:structuredClone(d.spec),state:'candidate',result:{...structuredClone(d.result),png:file('candidate',d.result.png)},...(d.composition?{composition:{version:1,inputs:d.composition.inputs.map((i,n)=>({...i,source:file(('input-'+n) as SpriteFileRole,i.source)}))}}:{})}
 const manifest:SpriteArchiveManifest={version:1,id:d.id,draft:{...draft,...(d.generation?{generation:structuredClone(d.generation)}:{}),...(d.protagonistIdentity?{protagonistIdentity:structuredClone(d.protagonistIdentity)}:{})},files};assertSpriteManifest(manifest);return {manifest,payload}
}
export function assertSpriteManifest(v:any):asserts v is SpriteArchiveManifest{
 if(!v||Object.keys(v).sort().join(',')!=='draft,files,id,version'||v.version!==1||!spriteArchiveId(v.id)||!Array.isArray(v.files)||![2,4,5].includes(v.files.length)||JSON.stringify(v).length>5500)return fail()
 ...
 if(!d||Object.keys(d).some(k=>!['version','id','revision','createdAt','parentId','source','sourceName','sourceKind','deviceStateSet','spec','state','result','composition','generation','actorPatch','protagonistIdentity'].includes(k))||d.version!=='sprite-draft-1'||d.id!==v.id||!integer(d.revision,0,1000000)||!integer(d.createdAt,0,Number.MAX_SAFE_INTEGER)||d.parentId!==undefined&&!spriteArchiveId(d.parentId)||d.source!=='source'||typeof d.sourceName!=='string'||d.sourceName.length>100||!['actor','states'].includes(d.sourceKind)||d.deviceStateSet!==undefined&&d.deviceStateSet!=='repair'||d.state!=='candidate'||!s||!r||r.png!=='candidate'||r.algorithm!==(s.backgroundMode==='magenta'?'magenta-matte-unmix-1':'neutral-matte-unmix-1'))return fail()
 if(d.protagonistIdentity!==undefined){if(d.sourceKind!=='actor'||s.kind!=='actor')return fail();assertProtagonistIdentity(d.protagonistIdentity,v.files.find((f:SpriteFile)=>f.role==='source')!.sha256)}
 if(d.generation!==undefined)assertSpriteGenerationSource(d.generation)
 if(Object.keys(s).some(k=>!['columns','rows','cellWidth','cellHeight','foot','kind','backgroundMode','sourceAnchors','neutralMin','chromaMax','matteSeeds'].includes(k))||!['actor','states'].includes(s.kind)||s.columns!==(d.deviceStateSet==='repair'?2:3)||s.rows!==(s.kind==='actor'?4:1)||!integer(s.cellWidth,8,1536)||!integer(s.cellHeight,8,1536)||!['alpha','pale-neutral','magenta'].includes(s.backgroundMode)||!integer(s.neutralMin,180,255)||!integer(s.chromaMax,0,35)||!s.foot||Object.keys(s.foot).sort().join(',')!=='x,y'||!integer(s.foot.x,1,s.cellWidth-1)||!integer(s.foot.y,4,s.cellHeight-1))return fail()
 ...
 if(s.matteSeeds!==undefined&&(!Array.isArray(s.matteSeeds)||s.matteSeeds.length>64||s.backgroundMode==='alpha'||s.matteSeeds.some((p:any)=>!p||Object.keys(p).sort().join(',')!=='x,y'||!integer(p.x,0,source.width-1)||!integer(p.y,0,source.height-1))))return fail()
 ...
 if(d.composition){const c=d.composition;if(d.sourceKind!=='states'||s.kind!=='states'||Object.keys(c).sort().join(',')!=='inputs,version'||c.version!==1||!Array.isArray(c.inputs)||c.inputs.length!==s.columns||roles.size!==2+s.columns)return fail();for(let n=0;n<s.columns;n++){const i=c.inputs[n],f=v.files.find((f:SpriteFile)=>f.role==='input-'+n);if(!i||Object.keys(i).filter(k=>k!=='generation').sort().join(',')!=='column,columns,source,sourceName'||i.source!=='input-'+n||!f||typeof i.sourceName!=='string'||i.sourceName.length>100||!integer(i.columns,1,12)||!integer(i.column,0,i.columns-1)||f.width%i.columns||f.width/i.columns!==source.width/s.columns||f.height!==source.height)return fail();if(i.generation!==undefined)assertSpriteGenerationSource(i.generation)}}else if(d.actorPatch){
 const p=d.actorPatch,b=v.files.find((f:SpriteFile)=>f.role==='input-0'),f=v.files.find((f:SpriteFile)=>f.role==='input-1')
 if(d.sourceKind!=='actor'||s.kind!=='actor'||d.deviceStateSet||Object.keys(p).sort().join(',')!=='base,column,frame,row,version'||p.version!==1||!integer(p.row,0,3)||!integer(p.column,0,2)||roles.size!==4||!b||!f||b.width!==source.width||b.height!==source.height||f.width*3!==source.width||f.height*4!==source.height)return fail()
 for(const [part,role]of [[p.base,'input-0'],[p.frame,'input-1']])if(!part||Object.keys(part).sort().join(',')!=='source,sourceName'||part.source!==role||typeof part.sourceName!=='string'||part.sourceName.length>100)return fail()
 }else if(roles.size!==2)return fail()
 if(d.actorPatch&&d.composition)return fail()
}
...
export async function restoreSpriteManifest(manifest:SpriteArchiveManifest,read:(file:SpriteFile)=>Promise<Uint8Array>):Promise<SpriteDraft>{
 ...
 if(d.actorPatch){d.actorPatch.base.source=files.get('input-0');d.actorPatch.frame.source=files.get('input-1')}
 return d
}
