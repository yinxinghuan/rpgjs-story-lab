...
import {assertDeviceReview,sameDeviceGeometry,type PublishedDevice} from './device-publication'
 ...
 async function operate(action:'save'|'list'|'restore'|'cancel'|'publish',record?:SpriteArchiveRecord){
  ...
  try{await navigator.locks.request(spriteDatabaseName(location.href),{ifAvailable:true},async lock=>{
   ...
   if(action==='save'||action==='publish'){
    ...
    await loadRecords();setMessage(action==='publish'?t('设备版本已发布，可用于新的独立旅程。','Device version published for a new separate journey.'):t('原图、处理参数和候选已在线保存。','Originals, preparation settings and candidate saved online.'))
   }else{
