...
export default function SpriteCloudPanel({repo,draft,busy,setBusy,onRestore,locale}:{repo:SpriteDraftRepository;draft?:SpriteDraft;busy:boolean;setBusy:(v:boolean)=>void;onRestore:(d:SpriteDraft)=>Promise<void>;locale:'zh'|'en'}){
 ...
 async function loadRecords(){const rows=await cloud.list(),entries=await Promise.all(rows.map(async r=>[r.manifest.id,await cloud.publication(r.manifest.id)] as const));setRecords(rows);setPublications(Object.fromEntries(entries.filter(e=>e[1]!==null)) as Record<string,PublishedDevice>)}
 async function operate(action:'save'|'list'|'restore'|'cancel'|'publish',record?:SpriteArchiveRecord){
  ...
  try{await navigator.locks.request(spriteDatabaseName(location.href),{ifAvailable:true},async lock=>{
   ...
   if(action==='save'||action==='publish'){
    ...
    if(action==='publish'){const c=await inspectDeviceMapCandidate(current,current.id,decodeSpritePixels);assertDeviceReview(current.deviceReview,c.png.sha256);if(!sameDeviceGeometry(c,current.deviceReview.geometry))throw Error('DEVICE_REVIEW_REQUIRED');await cloud.publish(current)}else await cloud.save(current,(done,total)=>setMessage(t(`正在保存素材 ${Math.round(done/total*100)}%`,`Saving art ${Math.round(done/total*100)}%`)))
    await loadRecords();setMessage(action==='publish'?t('设备版本已发布，可以查看固定版本素材。','Device version published. Its fixed-version image is available.'):t('原图、处理参数和候选已在线保存。','Originals, preparation settings and candidate saved online.'))
   }else{
 ...
 return <section aria-label={t('在线人物与设备','Online character and equipment art')}><h2>{t('在线人物与设备','Online character and equipment art')}</h2><p>{t('最多6份素材，每份原图和结果合计最多24MiB。来源记录保持私有；保存成功不等于画面检查通过。仍使用本浏览器制作身份，尚未绑定平台账户。','Up to six records, each containing at most 24MiB of originals and results. Sources stay private; storage success does not mean visual approval. Access uses this browser’s creation identity, not a platform account yet.')}</p><button disabled={busy||draft?.state!=='candidate'} onClick={()=>void operate('save')}>{t('在线保存当前人物或设备','Save current character or equipment online')}</button><button disabled={busy} onClick={()=>void operate('list')}>{t('查看在线人物与设备','View online character and equipment art')}</button>{draft?.deviceStateSet==='repair'&&<button disabled={busy||!reviewed} onClick={()=>void operate('publish')}>{t('发布已检查的设备版本','Publish reviewed device version')}</button>}{draft?.deviceStateSet==='repair'&&!reviewed&&<p>{t('先到地图完成设备检查，再发布版本。','Complete device checks in the map before publishing.')}</p>}{message&&<p role="status">{message}</p>}{error&&<p role="alert">{error}</p>}{records?.length===0&&<p>{t('还没有在线素材。','No online art yet.')}</p>}{records?.map((r,i)=><div key={r.manifest.id}><p>{i+1} · {r.manifest.draft.sourceName} · {r.state==='ready'?(publications[r.manifest.id]?t('已发布设备版本','Published device version'):t('已保存','Saved')):t('上传未完成','Upload incomplete')}</p>{r.state==='ready'?<button disabled={busy} onClick={()=>void operate('restore',r)}>{t('取回此素材及原图','Restore art and originals')}</button>:<button disabled={busy} onClick={()=>void operate('cancel',r)}>{t('清除未完成上传','Remove incomplete upload')}</button>}{publications[r.manifest.id]&&<a href={import.meta.env.MODE==='cloud-preflight'?`./?story=original&device_release=${encodeURIComponent(publications[r.manifest.id].id)}`:deviceReleasePath(publications[r.manifest.id].id)+'/file'}>{import.meta.env.MODE==='cloud-preflight'?t('用此设备进入独立旅程','Enter a separate journey with this device'):t('查看已发布的设备图','View published equipment')}</a>}</div>)}</section>
}
