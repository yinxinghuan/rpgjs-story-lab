...
export async function inspectActorMapCandidate(draft:SpriteDraft,id:string,decode:(png:SpritePng)=>Promise<PixelRaster>):Promise<ActorPreview> {
 ...
 const pixels=await decode(r.png)
 if(pixels.width!==r.png.width||pixels.height!==r.png.height||pixels.rgba.length!==pixels.width*pixels.height*4)return invalid()
 ...
 return {id:'creator-actor-'+id,png:r.png,width:r.png.width,height:r.png.height,scale,baselines:Array.from({length:4},()=>[s.foot.y,s.foot.y,s.foot.y]),centers:Array.from({length:4},()=>[s.foot.x,s.foot.x,s.foot.x]),frameBounds}
}
