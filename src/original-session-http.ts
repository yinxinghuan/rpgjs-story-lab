...
export function originalSessionHttp(storage:Storage,lock:SessionLock,request:typeof fetch=fetch,apiBase=getGameApiBase(),prepareScene?:(plan:OriginalActionPlan)=>Promise<void>,backgroundRelease?:string,deviceRelease?:string){
 ...
 const api:typeof transport=(path,body)=>{
  ...
  return transport(path,path==='/sessions'&&body?{...body as object,...(backgroundRelease?{backgroundRelease}:{}),...(deviceRelease?{deviceRelease}:{})}:body)
 }
 const continuation=deviceRelease?prefix+'device:'+deviceRelease+':background:'+(backgroundRelease??'default')+':':backgroundRelease?prefix+'background:'+backgroundRelease+':':prefix
