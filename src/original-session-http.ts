...
export function originalSessionHttp(storage:Storage,lock:SessionLock,request:typeof fetch=fetch,apiBase=getGameApiBase(),prepareScene?:(plan:OriginalActionPlan)=>Promise<void>,backgroundRelease?:string,deviceRelease?:string,actorRelease?:string){
 ...
 const api:typeof transport=(path,body)=>{
  ...
  return transport(path,path==='/sessions'&&body?{...body as object,...(backgroundRelease?{backgroundRelease}:{}),...(deviceRelease?{deviceRelease}:{}),...(actorRelease?{actorRelease}:{})}:body)
 }
 const continuation=actorRelease?prefix+'actor:'+actorRelease+':device:'+(deviceRelease??'default')+':background:'+(backgroundRelease??'default')+':':deviceRelease?prefix+'device:'+deviceRelease+':background:'+(backgroundRelease??'default')+':':backgroundRelease?prefix+'background:'+backgroundRelease+':':prefix
