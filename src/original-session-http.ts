...
export function originalSessionHttp(storage:Storage,lock:SessionLock,request:typeof fetch=fetch,apiBase=getGameApiBase(),prepareScene?:(plan:OriginalActionPlan)=>Promise<void>,backgroundRelease?:string,deviceRelease?:string,actorRelease?:string,fanRelease?:string){
 ...
 const api:typeof transport=(path,body)=>{
  ...
  return transport(path,path==='/sessions'&&body?{...body as object,...(backgroundRelease?{backgroundRelease}:{}),...(deviceRelease?{deviceRelease}:{}),...(actorRelease?{actorRelease}:{}),...(fanRelease?{fanRelease}:{})}:body)
 }
 ...
 const continuation=fanRelease?baseContinuation+'fan:'+fanRelease+':':baseContinuation
