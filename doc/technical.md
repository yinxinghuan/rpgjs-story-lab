...
当前只通过固定来源hash的候选处理脚本使用，尚未向玩家制作页开放点选。neutral-09的确认点为153/150，处理另存candidate-alpha-gap.png；旧候选和原图均保留。静态比较确认只改变第一格678个像素，第二格完全不变。动态装配仍需桌面解锁后复验，不能据此准入正式主角。
...
`spritePreviewUrl` 对浏览器不接受AbortSignal的 `Image.decode()` 增加15秒等待上限，并转接调用方取消信号。失败释放对象URL，完成清理计时器和监听器；迟到的解码不恢复旧预览。专项测试模拟永久挂起、超时清空图片src、迟到完成及下一次成功，1/1通过，TypeScript通过。此条仅覆盖预览解码，不宣称整条后处理链的所有解码等待均已验证。
