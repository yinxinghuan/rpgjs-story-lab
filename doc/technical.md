...
## 交谈呈现节奏（2026-09-17 试玩修正）

`old-street-dev.tsx` 以当前对白页的 block id 组合为呈现 key；等待期间隐藏选择的视觉与可访问内容但保留布局位置，最终对白页出现 900ms 后开放选择。关闭、翻页、换人、忙碌或报错均取消旧计时器；恢复入口不延迟。提交自由表达收起输入框，回应/翻页只重置面板内部滚动，不改地图相机、请求协议、叙事状态或存档。CSS 将 NPC 引文、玩家话语和纸色选择分开，减少动态效果关闭淡入。实际时序与手机尺寸证据见 `dialogue-rhythm-review-20260917.md`。
...
## 2026-09-17 地图方向与界面命名修正

`old-street-map-layout.ts` 从 `oldStreetDoors()` 和 `oldStreetFloors` 计算当前场景的出口投影，地图不再使用手写全街节点坐标。阈值位置保持比例、相邻地点按出口侧摆放，同侧多出口保持先后且避让标签。远处地点列表复用已知地图及 BFS，只高亮真实下一出口；选择目的地不修改存档或移动人物。地图/背包/存档等中英可见标题、无障碍名称和操作说明同步。

## 2026-09-17 图文背包

`old-street-journal-art.tsx` 为展示注册表：已有时钟/推车/拼合照片缩略图，已知人物对应当前存档美术版本的静止帧上身裁切，以及物品/记录类别 SVG 图标。只从 `oldStreetJournal` 已过滤的条目渲染，不从素材表建立人物或物品。缺图和加载失败回退语义图标；未拼合照片不显示完整图。动态照片复用 `useCommittedStreetPhoto` 的当前旅程内容哈希核验，背包打开时启用，卸载释放 Blob，不读取其他旅程。

`OldStreetJournalView` 保留四类页签，目标页有计数捷径；物品图文列表常显用途，人物头像/身份常显、经历展开，线索按短标题展开。展示数据与存档 reducer 不变；没有新增道具使用或后端写接口。


## 动态背包图片与轻量互动提示（2026-09-17）

- `src/old-street-journal-media.ts` 从权威存档中选择实际持有且有描述的新增物品、已登场且有外观描述的动态人物。固定物品/人物继续复用现有图与精灵裁切。稳定对象 ID 决定同一旅程的图片身份，首次准入的描述冻结后不随读档重画。
- `server/old-street-journal-media.ts` 在现有旅程数据库增加媒体任务与 PNG 分片表，按 owner、journey、对象 ID 隔离。图片是旁挂资源，不修改道具、关系、通路或故事状态。使用现有平台媒体客户端生成 512×512 PNG，保存任务 ID、请求 UUID、哈希与真实文件；并发租约避免重复请求，恢复使用原请求/任务，明确终态后才允许用户重试创建新请求。
- Worker 与本地适配器提供同一 `journal-art` / `journal-art-file` 路由。行动提交完成后后台排队；背包打开时同步并每8秒检查未完成任务。关闭面板停止客户端轮询，已发出的后台工作与任务状态保留，后续打开可继续恢复。终态错误保留图标和条目，并提供重试；断网保留重新加载入口。不得把媒体失败升级为行动失败。
- `src/use-journal-media.ts` 校验文件哈希并为 UI 生成 blob URL；关闭/换旅程回收 URL。固定尺寸槽原位替换，不遮挡探索。动态肖像不等于实体 NPC：`old-street-mediated-cast.ts` 只允许可信内容适配器在同一提交中记录可见介绍、人物及既有书信/记录媒介绑定；空间编译仍拒绝未绑定人物。当前没有新增自由输入任意造人、走动精灵或碰撞自动生成的入口。
- `OldStreetObjectPreview` 为静态内嵌状态图，无点击放大层；钟表观察、照片拼合等解谜模块继续保留自身的操作界面。
- `old-street-interface.css` 将当前互动对象的矩形描边改为右上侧小菱形，普通物品低对比、当前目标提亮；点击区域、实体目标和右下行动入口保持原合同，键盘聚焦同样强调菱形。

验证：`_qa/old-street-journal-media.test.ts` 覆盖持久恢复、原请求复用、失败回退、并发、媒介人物准入隔离与真实 HTTP 文件合同。`_qa/dynamic-journal-server.ts --live` 使用独立合成数据库进行真实平台生成，不读取玩家旅程；具体结果见 `dynamic-backpack-review-20260917.md`。
...
### 屋顶构件尺度与颗粒（2026-09-17修订）
默认街口与合住院恢复绿金属/蓝灰瓦，新增陶瓦和沥青不进入默认场景。旧屋顶`worldWidth=80`（原40），保持36/40单位可见屋面，朝内檐沟锚定并裁切外侧。`RoofMaterialSurface`仍以每地图单位2像素生成一次缓存，但Canvas改为最近邻采样，保留原图颗粒。尺度与采样各自独立；未改变道路、碰撞或相机。

### 完整通道素材接入（2026-09-17）
`old-street-entrance-art.tsx`统一渲染三张平台生成后处理图集：木门正面开闭、俯视侧门开闭、石巷/洗衣布帘。`old-street-door-view.tsx`仍从`oldStreetDoors()`和权威facts选择位置、状态与是否允许出现；阶梯及木箱门禁保留。地面通路可按连接方向旋转，立面木门保持向屏幕上方投影。
南侧木门在背景绘制完整图，墙体前景之后重绘门框、檐头和门扇轮廓，门槛不覆盖人物脚底。沿用原地图、碰撞和行动接近点。图集通过`oldStreetEnvironmentArt`进入Vite资源打包与环境预加载，路径随任意部署子路径解析。
原图及生成请求位于`doc/entrance-art-20260917/`；`scripts/prepare-entrance-art.ts`按源SHA256验证，去除洋红底并对齐门槛，保留源文件。正面洞口及门缝内封闭底色区域使用人工审查过的种子点清理。准备后的图集位于`prepared/`，含过程与hash记录。

### 入口墙体遮挡更正与方案对照
上段初轮“墙体之后重绘门框”已被用户指出前后关系错误，现更正为：地面 → 入口底图 → 后/侧墙 → 人物 → 南门结构 → 前墙 → 屋檐。墙体应压住入口边缘；不能让素材覆盖前侧墙来掩盖错位。`old-street-dev.tsx`与真实renderer检查页同步这一顺序。
`_qa-entrance-options.html`为隔离的本地视觉比较，不进入生产构建入口、不读写存档。A/B/C的正面候选为新生成的宽门，侧面分别使用已认可的石通路、混凝土凹口与顶视石廊素材；侧面后两者只作为水平投影的概念对照，仍须检查局部受光及轮廓，未准入运行时。页面共享真实场景组件和主角原尺寸图集，比较四个方位；人物为固定参照，不代表已完成候选门的动态穿越验证。

### 门素材库的平台来源更新（2026-09-18）
`src/material-library/door-catalog.json`按稳定ID记录原认可图与平台替代图，`preferredByPose`给出内收斜开/90度优先项；`door-materials.ts#getPreferredDoorMaterial`读取优先项并校验朝向。`scripts/prepare-platform-doors.ts`保留平台原图与hash，清理已检查背景并平移对齐脚点；不变形图像。`_qa-platform-doors.html`比较同镜头墙体遮挡；运行`node --import tsx --test _qa/door-materials.test.ts`检查库完整性。首轮仅入库；现已由下述运行接入替换默认游戏入口图集。


### 2026-09-18 通道运行接入与回家正门

`old-street-environment-art.ts`默认正面/侧向图片已替换为平台新图；新增内收门和回家门，房间下载清单包含实际依赖。`OldStreetEntranceArt`消费door-catalog的开闭列、比例、门槛脚点与南侧前景区域。修表铺后门固定内收，其余正面房门90度；W/E专用侧向图不旋转正面立面。

侧向不是把整张俯视图贴在地面：`SideDoorway`保留门槛地面层，使用已生成木框纹理分别绘制有高度的远侧/近侧门框。高度取室内56、室外24的现有墙体合同；远框在角色后、近框在角色前。`OldStreetRoomWalls`侧墙顶面按同一高度投影，侧门缺口仍由同一门位分割墙段；`OldStreetRoomForeground`绘制近墙段端面，最后由原屋檐覆盖。侧向纹理生成成功不等于模型直接生成了完整正确立体结构，厚度和遮挡由运行渲染组合实现。侧路和布帘保留原地面差异，共用这一厚度结构；阶梯未改造。

回家实体仍为`street-exit`/`oldstreet:leave`，位置改到street底边y544，接近点y496；南墙洞口读取同一实体x。独立深青双扇正门随departed事实开闭，普通按钮底板改成门标签，靠近后统一行动按钮进入原有确认流程。没有增加新转场、提前结局或新的存档分支。

本地代表验证：真实RPG-JS角色在修表铺底门洞内、广场两侧门及回家门渲染；正常试玩工作棚→屋顶→照相馆→街口转场。16项素材哈希/朝向、墙洞与寻路/原结局规则测试通过。未发布线上，侧门最终美感尚待用户实际判断。
...
### 侧向短通道本地候选（2026-09-18）
`old-street-side-passage.tsx` 从权威 laundry-back 门位置取得锚点；平台 PNG 单图去底，保持原尺寸，按端部落地点间距 251 源像素对应 56 世界单位。前景仅重复近端木构件。`old-street-room-walls-view.tsx` 将洗衣店东侧墙顶面向上投影，远墙补端面，近墙在角色层之后渲染。原转场与碰撞不变。仅一个代表场景，尚未推广侧门。

### 2026-09-18 侧通道最终合同（替代上面的单场景候选）
`old-street-side-door-config.ts`集中连接样式、上侧铰点、开向和48单位门扇宽度；仅类型引用space以避免初始化循环。`openSideLeafBody`给打开且朝当前房间展开的门板返回48×4地面碰撞；移动、寻路、权威接近判断共用`oldStreetWalkable`。闭门沿用边界和gate事实，纯通道不添加门板碰撞。移开的院内木箱改由东侧接近，避免其旧调查站位与后门门板重叠。
...
### 连续地图与拖动（2026-09-18）
`old-street-map-layout.ts` 的 `OLD_STREET_ATLAS` 保存固定节点坐标，边界随登记地点的最大坐标计算。`OldStreetMapView` 从权威存档读取已探索节点与开放连接，统一画在连续图层；拖动和双指缩放只改 UI 相机，不修改存档。默认 16px 地名、独立地图画布、定位当前房间；全图为可选缩小总览。新增固定地点需登记稳定坐标，尺寸随边界增长；尚未实现任意模型生成地点的自动空间排布，不将其宣称为已验证能力。下一步出口指引仍取真实 `oldStreetDoors()`，全图跨层连接是路线示意。

验证：地图访问/路线两项现有单元测试通过，TypeScript 检查与 cloud 模式前端构建通过；真实组件合成页面检查 390×844 中文、320×568 英文、单地点隐藏、选中远处地点完整路线、鼠标指针拖动、缩放按钮与定位。截图在 `_qa/ui/continuous-map-20260918/`。双指逻辑已实现，但当前内置浏览器不支持触控事件注入，未将 iPhone 双指实机体验标为通过。本轮未发布线上。

### 正式旧街发布版本检测（2026-09-18）
Vite `gameReleasePlugin` 对 src/public/server、入口、依赖清单和 Worker 源文件生成内容指纹，同时嵌入客户端并输出相对路径 `release.json`。不依赖 Git 元数据，cloud / Pages 同源码得到相同版本；只更新公开图片也会改变版本。`useGameRelease` 在启动、前台恢复、网络恢复和前台每 60 秒进行无缓存同源读取，不附带登录凭据。连续两次收到相同的新版本（首次发现后 2.5 秒复核）才提示。错误、离线、非法清单不会阻断游玩。

正式旧街 `OldStreetDev` 等 busy 结束并已就绪或出现可恢复错误后，暂停角色/NPC并显示不可取消的 `GameReleaseNotice`；不自动 reload。按钮通过保留原路径、参数和 hash 的 `_release` 参数绕过入口缓存，不改任何存档或待确认队列。此机制只在已加载带检测代码的页面生效，不能远程改写已打开的更早版本页面；旧列车参照入口和制作页尚未接入此提示。
