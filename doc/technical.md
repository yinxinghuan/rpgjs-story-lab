...
只有在 Authority 显式装配 campaignGenerator 且创建选项为 `{campaign:'letter-trail-v1'}` 时才新建这种旅程。现有 HTTP 创建路由和玩家界面尚未开放该选项；默认创建与所有旧存档不变。开启后的取信反馈指向记录册，最终离开要求两段都完成；旧旅程仍可按原规则离开。下一步必须接入用户可操作入口、专用材料视图、自由输入映射与后台生成/等待衔接，再验收真实生成。当前 plan 事务会等待一次生成，尚未实现提前异步准备，不宣称渐进加载已完成。
