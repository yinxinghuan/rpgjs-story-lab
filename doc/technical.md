...
服务演进边界：已读取 `alteru-rpg-service/doc/给其他游戏制作任务的说明.md`（2026-09-10）。新的 Prolog 封装仅独立本机开发，暂无正式接口交付；当前游戏继续既有已验证链路，不提前采用新 API、不自动迁移存档，也不把现有 TypeScript 规则测试当作 Wilson 规则已登记启用的证据。将来接入须依据正式接口包和上线验证结果核对版本、身份、存档与客户端。
