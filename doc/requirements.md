...
现有 `StoryCartridge.statDefinitions` 强制恰好三个数值，新故事不需要三条生存数值；后续应让核心/界面允许空列表，逐项检查消费者，不能以填三个无意义数值绕过类型。`createInitialSave` 默认加电影式开场图块，也需要空间壳显式关闭或投射隔离，不能让等待新图阻塞第一步。以上为已核对代码的适配缺口，尚未修改。
