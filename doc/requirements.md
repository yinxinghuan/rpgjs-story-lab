## 旧照片的拍摄时机调查（2026-09-17）

新准备的完整调查采用明确的结构：固定维修事件与一次旧照片拍摄，两者谁先发生须查档案。程序提供两种合理顺序：先拍摄／冲洗，再排期／完成维修；或先完成／登记维修，再架设相机／拍摄。模型补具体照片名称、记录标题和支持的家具布置，不能自行排列因果或宣布答案。

仍然查两处资料、核对四条记录；两处线索合起来唯一确定顺序。调查结果记录拍摄在维修之前或之后，后续真实照片生成必须消费这个已查明的时机和同一维修对象。照片本身只描述可见细节，不能反过来声称独立证明日期。错误尝试可重做，已读证据保留。

不增加强制跑腿、房间或阅读步骤。旧档已存在的材料和档案保持原样，普通短旅程与已完成旅程不增加门槛。本项是一种支持的调查结构，不是任意剧情因果生成或完整时长达标。

## 调查内容必须用到档案证据（2026-09-17，实验目标）

观察：实际生成的“报修→检查→采购→更换灯泡”无需档案即可猜出顺序，削弱寻找记录的意义。新准备的调查应围绕同一事件中的两项可独立排期的工作，保留已知固定事件；问题两端不采用同一次维修的动机与完成、材料取得与安装等必然先后。历史可通过不同安排表现通行、使用或保存方面的取舍，但不把旧事等同于玩家当前已经改造地图。

玩法仍是查两处真实证据、核对四条记录、决定原件去向。两处证据合起来唯一确定答案，任一处单独不足；错误排序不消耗道具、不抹除已读证据。目标是让证据有用，而非增加步骤、提高阅读量或制造不合理反转。旧旅程已读内容不重写。新内容复核只拒绝明显可以靠常识完整推出的流水账，保留普通细节瑕疵；最多一次修正，不无限生成。

本项只改善历史调查的内容与推理，不宣称新增玩家分支或完成内容时长。玩家理解和趣味仍为待观察假设。

三批真实试验未达到稳定准入：仅追加提示与复核不能可靠保证历史因果和非显然答案。因此本段任意事件生成规则仅在明确的离线合成实验选项中使用；后续默认方案见上方“旧照片的拍摄时机调查”，不能将本实验结果写成正式能力已完成。

## 完整委托进入默认试玩（2026-09-17）

下一次整合发布后，正式主站首次进入和“另开一段探索”默认开启完整取信委托，包括档案调查、屋顶取底片和暗房显影；仍标注试玩，不宣称内容时长或最终美术已验收。旧旅程和待确认的新建请求按原版本恢复，不追加新任务，不删除或替换旧存档。Pages继续只作为静态镜像，不通过另一后台创建旅程。

新建旅程本身不触发生成；真实内容仍在探索到相应节点时准备。本地使用与正式Worker相同的默认入口验证新建、恢复和旧档保留，不重复完整路线。

## 已解调查的材料回看（2026-09-17）

调查完成后，回到纸袋前应先看到已经核对的结论，不继续提出旧问题或催促追查；原始寄存条可主动展开回看。带走／留下仍是现场独立选择，不自动替玩家决定。决定后显示同一旅程的当前目标，引导继续探索。未完成调查保持证据入口，旧版无调查纸袋保持原样。

点选目的地后，居民移动挡住旧路线时应保留原目的地并按最新碰撞绕行；每次点选最多重算两次，仍无路时停止并提示重新选择。不得穿过居民、瞬移或触发未到达的行动，暂停／手动移动仍取消寻路。

## 在探索途中准备必要内容（2026-09-17）

新完整委托拿到密封信后，后台开始准备寄存记录；玩家选定匹配记录后，后台准备同一事件的材料与档案。玩家可继续走动，不必到达下一处再开始等待。完成准备只意味着可阅读，不授予线索、道具、知识、房间入口或完成事实；仍须走到对应物件前主动阅读和操作。

每段只自动启动一次；失败不在走路／读档时反复重试，玩家在材料原入口重试。旧旅程不追加自动生成。明确返回的行动结果不能被后续后台准备失败变成“结果未知”。

## 完整委托的本地普通入口（2026-09-17）

本地完整试玩从正常游戏页开始，不需要调试菜单或预先完成档案。首次进入及“另开一段探索”创建取信、档案调查、屋顶底片、暗房与带回发现的同一委托；已存在旅程照常续玩，不把旧短旅程升级成新任务。线上仍维持当前发布版本，新增委托的生产准入另行完成。

未拿到底片或未走近冲印台时，被拒绝的操作应给出具体下一步，保持原进度并允许继续走动；确认拒绝不再当成网络结果未知。断网且尚未确认的提交仍按原恢复规则处理，不能误当失败丢弃。

## 重要物品特写（2026-09-17）

走近只显示右下行动，主动查看后，已有合格素材的重要物件显示实物预览；可放大观察，返回仍停留在同一物件的行动面板。抽屉和信件小格先复用闭合／打开／取走三态，不能用旧图继续显示已经拿走的物品。钟底、照片、显影保留各自操作型特写。普通家具不强制增加特写，不增加必经点击或故事门槛；素材失败不能阻止原有行动。代表性验证覆盖预览、放大、返回和状态变化，不逐个重复故事路线。

## 档案线索驱动实地取底片（2026-09-17）

仅新建的下一版完整委托将现有屋顶调查接到显影之前：还原同一份动态档案后，整理出的归档单指出对应底片在屋顶北侧柜子的双缺口纸套中；玩家沿正常门和楼梯过去，观察破损处、用现场或工作棚的长板搭路、取出底片，再回照相馆冲印。归档单属于现在的存放信息，不把历史维修事件说成当前地图发生的事情。

这段替代原来“照片已出现后再可选寻找底片”的顺序，让调查结果实际指导地图行动。可以提前探索屋顶和搭路，但没有档案线索不能辨认底片，没取到底片不能启动相关照片准备。找错/短板不足不扣资源。拿到后可带着冲印并保留，也可交给摄影师保存再冲印；照片自身仍可带走、留下或展示，两种原件不能混为一物。

目标提示和发现页始终可回看归档线索；已准备的素材不等于玩家已发现。中断后继续同一旅程，不重新抽取事件或归档来源。现有旅程保持旧顺序、结束条件和已完成状态，不自动升级。沿用当前B视角、双输入和现有纸套/柜子/木板素材，不新增窗口或强制交谈。

验证问题：前段调查是否给出明确地点与操作目的，实际寻路、取物、在手/交存两条显影路线和结局是否使用同一来源？用新旧旅程及恢复验证核心链，再走一条真实renderer路径。此为完整委托结构调整，不以多走几步证明45–60分钟、动态章节深度或新玩家理解已经达标。
...
### 开场与后台准备回访
首次委托提供移动、右下主动互动、随身回看目标的简短说明；旅程菜单可再次阅读操作方法。已有后台任务在随身内折叠展示进度与返回地点，离开场景后仍能获知就绪；就绪只给一次6秒短提示，不自动开窗、转场、承诺已观察内容或追加生成。断线后仍保留原任务，原地点提供重连/重试；结束/已准入的任务退出准备列表。同一放大台同时承载外借日志与暗房入口时，已读日志不再强制独占操作入口。


### 动态入口准入后的衔接
暗房准备完成后，放大台上的操作明确称“打开暗房门”。完成后收起物件面板，短暂显示后门已可通行；角色仍留在原处，由玩家走到后门并按统一行动按钮进入。不能把开放入口按钮写成已经进入房间，也不能只在后台更新可通行状态而无可见结果。准备失败、查询断线与准入拒绝分别保留原任务及明确恢复入口。
# 专注界面的输入归属（2026-09-17）

地图、行囊、旅程、解谜特写及结束探索确认打开时，人物和居民停止移动，方向键只归当前界面使用；关闭后保持原地，不续走之前的路线。结束确认使用和其他工具一致的模态窗口，默认聚焦“再逛逛”，Escape 返回探索。非模态的普通对话/物件面板仍允许玩家主动走开。
# 生成照片的实物去向（2026-09-17）

玩家带走亲手显影的旧街照片后，可在修表铺把它放在公共调查摘要旁。界面先说明：照片会从行囊移出、留在现场，回家只能转述；离开前可到原处取回。展出同一旅程的真实照片，非默认插图；撤下摘要时照片一并收回，不丢物品。私有旅程中的“公共记录”只是游戏世界内的公开，不向其他真实玩家发布。这个可逆取舍不增加通关门槛，不用额外跑腿强行延长游戏。

验证重点：带回/展出两路的现场、行囊、居民知识与结局一致；展出后刷新及返回能恢复；自由输入与按钮走同一权威入口。它验证生成内容的实际后果，不代表动态章节深度或45–60分钟已经达标。

### 动态档案间：交替通路（实施合同）

在已有就地调查路线中加入一种房间解法：索引和工作日志并排，滑动储物架在两个站位之间移动，一次只露出一处记录。玩家可先读日志再挪架读索引，也可先挪架读索引、再移回查日志；已经读过的证据始终保留。移动的是实际家具与碰撞，不能从架子背后远程查阅。提示只说明哪处入口被挡住，不替玩家提交最终结论。

沿用靠近→右下行动→选择操作的结构，不新增常驻窗口。两种架位都能接近储物架、整理桌和出口，误操作可撤回，没有消耗与惩罚；不额外要求重复来回次数。生成器可选普通移架、交替通路或无架，交替通路仅适用于两份记录均在本屋的情况，不能和日志外借混用。房间确定后保存，不在读档时重排，旧旅程原样保留。

核心验证：两种读取顺序都能取得两份证据；移动/恢复改变实际可达目标；关闭/刷新恢复同一架位和已知证据；随后可完成既有调查并沿自然门离开。此实验验证空间规划差异，不把增加一个机关算成完整动态章节或目标时长达标。玩家理解与乐趣仍待整体试玩判断。

## 动态背包素材补齐（2026-09-17）

已被旅程确认、实际持有的独特物品，以及已介绍且有外观描述的动态人物，可以后台生成背包图片。固定对象优先沿用已有素材；普通便笺等使用类别图标。新图不影响物品用途、角色关系、剧情推进或地图准入。图片准备期间保留图标与说明，完成后原位替换，失败可重试。重新进入同一旅程沿用同一任务和成图，不重复生成。人物肖像只用于人物条目，不自动变成可行走NPC。

普通物品特写直接嵌在互动面板，不提供二次放大和重复详情。拼图、观察钟底等真正有操作的近景保留。

### 轻量互动标记

物品与人物使用小菱形点提示可互动；当前靠近的目标提亮，并与右下行动按钮保持一致。移除包围对象的高亮框，保持原点击区域和交互距离。标记不跳动、不引起布局位移；键盘聚焦同样用标记加强显示。

## 室内墙体与遮挡统一（2026-09-17）

七个室内区域（修表铺、洗衣店、照相馆、工作棚、地下室、暗房、档案间）共用上墙、侧边和前景墙的空间规则。上下墙总高度均为64地图单位；前景墙向地面内投影56单位，加8单位墙厚，人物被挡住时局部淡化。实际门/楼梯留通口，已开放的动态入口同步开槽。街口、院子和屋顶保持室外空间，不能为统一而封成房间。仅改呈现，不更改碰撞、门落点或存档。

### 建筑入口与等高墙（2026-09-17补充）
室内上下墙保持相同高度；人物走到下墙后时，局部墙面淡化以便判断位置。广场和适合的院落入口应可直接辨认为檐下通路，开门与锁门有不同实物状态，不靠测试文字解释；保持现有出入口位置和操作方式。

### 高频户外枢纽的建筑围合
根据实际地图连接，街口（三条通路）与合住院（五条通路）使用两侧可见的完整邻接建筑屋面，让玩家理解店铺/院落的位置；合住院不能仅给门加孤立短雨棚。北侧地下阶梯、南侧街口通道保持清楚。室内使用剖面墙；可探索屋顶保留开阔边缘，不画可能误导为断口绕行道路的邻接屋面。

### 全场景上下建筑边界补齐
按玩家补充要求，十个场景均有上沿实体墙面和下沿前景遮挡。街口、合住院使用院墙，屋顶使用女儿墙；室内继续等高墙。室外上下总高一致为32单位（含8单位压顶），室内64单位。通路按真实门/阶梯留口；街口南侧沿“回家”位置留出口。边界呈现不改地图、行动和镜头。

### 屋顶素材库
屋顶材质纳入可复用目录，记录原图、来源、裁切、平铺周期、地图尺寸、像素密度及准入状态。街口与合住院按建筑分配样式，同一修表铺前后保持一致。粒度调整不放大瓦片、不改可走范围。

### 屋顶密度反馈修正
玩家明确否决新增陶瓦/沥青在40地图单位内的密集排列。默认试玩已撤回两种新材质，街口/合住院恢复绿金属与蓝灰瓦。不得将减少噪点称为减少瓦片密度。新增材质只留在独立对照页校准单块瓦片实际尺寸，通过实景评审前不重新默认启用。

### 入口专项重绘与底部门墙衔接
门和通道为高频空间识别元素，重新用平台生成完整入口候选，避免仅靠几何块与平铺木纹拼装。必须覆盖木门开闭、侧向入口、开放巷道和洗衣帘门；已可接受的石阶继续沿用。门扇、门框与墙体应有正确前后层次，南侧门框不被前景墙切断。先检查候选透视并接入真实场景，不合格图不替换。

### 屋顶构件尺度的明确修正
用户所说“密集”是瓦片与排水管实际显示过小，不是像素噪点。默认旧金属/蓝灰瓦纹理的世界宽度由40改为80，构件尺寸加倍；两侧屋面占地仍36/40，保持檐沟朝内锚定，从建筑外侧裁切多余纹理。道路与镜头完全不变。

### 通道方案第二轮（2026-09-17用户评审）
石阶和石巷已被认可并保留；第一轮门尤其侧门被否决。侧门不能像地面木栏或孤立门框，必须融入墙体和屋檐。并列制作A嵌墙凹门洞、B实用卷帘门、C檐下短门廊三种方案，用相同场景、比例和角色比较正面/侧面及开闭；先独立评审，不直接反复覆盖默认门。

### 门素材平台生产验证（2026-09-18）
用户已接受简框内收斜开与90度两种姿态，并要求平台媒体服务生成类似效果、更新门库。使用已接受图作为编辑参考，按当前可接受范围检查平行投影、开闭一致、去背景、门扇不伸入墙体以及人物比例；不要求像素级复刻。通过者优先入库，原认可图保留并记录来源，失败生成不得充作已通过90度款。

2026-09-18 通道收尾：默认试玩采用已准入新门；侧通道有前后墙段与门框厚度，不以平铺贴纸代替立体开口。广场底端“回家”改为独立正门，走近后统一行动按钮确认，保留原结局条件。
...
### 侧向出入口表现（2026-09-18）
所有侧边门与纯通道共用室内可见的短过门石，薄地板边沿、场景上墙端面和下墙前景遮挡共同表示空间厚度。有门的建筑采用外开门；室外看到打开门扇的正面，允许店名与行业符号；纯通道不显示门扇。当前洗衣店短通道透视已由用户认可，其他侧入口待按此统一。

### 2026-09-18 侧门最终交互
侧边入口统一保留短过门石；纯通道不装门扇，有门的连接按场景设定内开或外开。门扇靠上侧门框铰接，打开后仍为实体障碍，人物从净空通道通行。商铺门以自然的玻璃字样和店内细节提示行业。门扇宽48地图单位，对应56单位门洞；源图等比展示。

### 照片查看与放置修正（2026-09-18）
- 随身旧街照片可从背包展开，查看当前旅程原图、已发现内容和放置地点；未解开的照片不提前显示完整答案。
- 修表铺公共记录册展示放置入口；未写入调查摘要时显示原因及先行操作，不能隐藏按钮。档案间整理桌明确提示照片应放在修表铺。
- 沿用现有放置、取回和存档规则，不自动公开摘要，不改写玩家选择。

### 公共记录册的信息结构（2026-09-18）
- 玩家首次打开即知道两项用途：查找寄存材料、留下调查记录。前者比对两项特征，后者先写经过再选择放照片；两者不混在长段说明中。
- 默认页随调查进度选择；原始记录可以随时回看。未查清经过时，留下记录页明确指向档案间整理桌；不把可选留档误写为结局必要条件。
- 每项记录明确状态、结果和主操作：经过未写入/已写入，照片未取得/在暗房/背包携带/已放置。撤下经过同时取回照片的后果必须在操作前显示。
- 中文英文、320×568 和390×844；触控按钮≥44px，正文16px，顶部用途切换与关闭可达，内容区独立滚动。理解效果仍需玩家试玩验证。

### 2026-09-18 连续地图修正
地图默认展示全部已探索地点与连接，保留稳定位置，不因当前房间变化拆成附近地点。选目的地高亮完整可通行路线；下一步出口方向继续取真实门数据。未探索地点不提前揭示。支持拖动、双指缩放、缩放按钮、全图和当前位置；地图不传送玩家。验证 390×844、320×568 中英文及只探索一个地点。

### 发布更新提示（2026-09-18）
正式旧街游戏每次打开、回到前台、恢复联网及前台每 60 秒检测发布版本。确认新版后要求刷新继续，不提供绕过入口；当前提交结束后才显示提示，不直接自动重载。刷新保留已有旅程与持久化待确认请求。网络失败继续现有游戏，不误报更新；开发热更新不触发正式提示。

### 2026-09-18 档案谜题理解改进（本地评审）
保留已确定的探索与权威判定规则，先改善理解再评选 UI 风格。玩家须看清：当前要找什么、已有哪两类证据、自己的判断、已确认结果及下一步。
- 寄存条：选择候选只进入并排比对，标记/包扎分别同栏展示；明确确认后才提交。不能提前自动标出答案。
- 事件排序：已查阅资料显示「先发生 → 后发生」关系；未查阅只显示来源与寻找提示。四个有序槽位常在，点事件放入，可撤回或上移，不靠拖拽完成必需操作。已收集来源数与待放事件数可见；失败保留选择。
- 记录册：调查结论直接可读，已确认时间线可回看；调查与照片各有状态、位置及动作。照片需先记下经过的依赖显式显示。可选留存不误导成回家必需条件。
- 生成文本直接使用现有结构字段，不从自由段落猜图形、不改变谜底。已确认信息留在背包线索中方便复查。
- 验证 390×844、320×568，中文/英文，缺资料、错误选择、正确提交、已记录/未记录、照片未带/携带/展示。实际玩家是否更好理解仍待试玩判定。

### 2026-09-21 体验补齐（实施中）

沿用国际受众、旧街现有故事所在地/人物身份及中英双语，不改变故事世界。继续探索、调查和跨次续玩的章节形态，不新增战斗或计分。

- 对话：已介绍人物的交谈面板可展开最近四轮完整问答；默认折叠，当前显示的一轮不重复列入历史。刷新、换房、换旅程后分别读取对应历史，旧存档无记录时显示真实空态。
- 声音：制作约 60 秒低干扰无唱词背景音乐、约 24 秒河边环境声；首个真实操作解锁，统一静音，离开页面暂停，回来后恢复失败仍可探索。已有距离脚步与确定性提示保留，正式启用前验听与音量检查。
- 新房间：先在现有受限房间扩展中交付稳定的可探索骨架，再并行补齐独立素材。用户确认采用自动检查后逐件显示、保留回退，不等待逐图人工确认。每件失败只保留对应体积，不阻塞出入；刷新恢复已完成素材，不重新生成全部。生成前后出口、人物落点、家具占地和谜题规则不变。提示仅为“世界正在加载，可以先四处看看”，不暴露内部参数。自动准入不代表人工美术验收。
- 恢复：目标是同一平台账号跨设备找回已有旅程，不公开访问凭据、不相信 URL 用户 ID、不创建第二套剧情权威。依赖服务端可验证平台身份；接口未确认前保留匿名续玩，不宣称账号恢复完成。
- 验证：390×844、320×568 中英对话历史及房间渐进状态；重开、刷新、延迟、失败、乱序返回；音乐解锁、静音及后台恢复。实体手机观感与平台账号验证单列，不由模拟测试代替。

### 可放大阅读的资料（2026-09-18）

有细节需要辨认的照片提供可点击缩略图与查看大图；照片 100%–300% 分档缩放并上下左右滚动。寄存条、候选记录、已查阅档案摘录及寄存材料支持独立阅读、100%–200% 调大字号。小图/摘录仍直接展示，展开阅读可选。关闭/返回只收起查看器，保留外层的选择、事件排列和调查状态；不把放大当成新发现、领取物品或谜题提交。没有解锁的档案不因查看器而提前展示。动态生成内容复用同一展示接口，不依赖额外生图；无图片的文档如实呈现文字，不虚构原件扫描图。
