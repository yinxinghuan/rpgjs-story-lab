...
脚步按实际移动距离触发，石街/木地板分材质，静止无脚步；初值相邻触发最小 120ms，之后按步态 QA 调整。抽屉/门栓以操作接触点播放约 150–300ms 的木/金属声；一次成功拾取用约 120ms 的轻提示；错误配对给约 100ms 的轻碰声，不用惩罚警报。对话开始不重复播放成功铃。河边环境声按场景淡入约 600ms；静音与系统减少动态偏好保留。基础脚步与拿取/物件音现以短合成音接入，无新录音；脚步每16世界单位且至少120ms触发，声音开关放在旅程面板并记住选择。河边环境声和背景音乐仍待制作，听感与iPhone恢复尚待验收。
