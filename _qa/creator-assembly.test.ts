...
test('four released slots assemble into one production journey; order independent resume, lost enrollment, private catalog and old journey retained',async()=>{
 ...
 const b=await bg.publish(d),a=await sp.publishActor(await assemblyActorDraft()),e=await sp.publish(await assemblyDeviceDraft()),l=await la.publish(await assemblyLayerDraft())
 const catalog=await loadAssemblyCatalog(f.api);assert.deepEqual(Object.fromEntries(Object.entries(catalog).map(([k,v])=>[k,v.length])),{background:1,actor:1,device:1,fan:1});assert.deepEqual(await bg.publication(d.id),b)
 const selection={background:b.id,actor:a.id,device:e.id,fan:l.id},href=assemblyJourneyHref(selection,catalog),q=new URL(href,'http://local').searchParams
 const connection=(query:URLSearchParams)=>originalSessionHttp(f.playerStore,lock,fetch,f.base,undefined,query.get('background_release')??undefined,query.get('device_release')??undefined,query.get('actor_release')??undefined,query.get('fan_release')??undefined)
 ...
 const check=()=>{assert.equal(originalActorRelease(h.assets)?.id,a.id);assert.equal(originalStarterRelease(h.assets)?.id,e.id);assert.equal(originalFanRelease(h.assets)?.id,l.id);assert.equal(originalBackgroundVersion(h.assets),b.id)};check()
 ...
 const stranger=creatorCloudTransport(new Memory(),lock,fetch,f.base);assert.deepEqual(await loadAssemblyCatalog(stranger),{background:[],actor:[],device:[],fan:[]});await assert.rejects(new CreatorCloudDrafts(stranger).publication(d.id));assert.equal((await fetch(f.base+'/api/creator/drafts/'+d.id+'/release')).status,401)
 await assert.rejects(loadAssemblyCatalog(async(p,b)=>{if(p==='/sprites')throw Error('NETWORK_DOWN');return f.api(p,b)}),/NETWORK_DOWN/)
 assert.throws(()=>assemblyJourneyHref({...selection,actor:b.id},catalog),/ASSEMBLY_RELEASE_UNAVAILABLE/);assert.deepEqual(readAssembly(JSON.stringify(selection)),selection);assert.equal(assemblyJourneyHref(emptyAssembly(),catalog),'./?story=original');assert.throws(()=>readAssembly('{"fan":"javascript:bad"}'))
