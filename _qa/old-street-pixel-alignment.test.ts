...
test('pixel drawer measured tabletop remains fixed in world coordinates across states',()=>{
 ...
 const [,sheet]=oldStreetPixelLayeredSheets('synthetic-image','drawer')
 const extents=Object.values(sheet.textures).map(texture=>{
  const f=texture.animations()[0][0];let left=512,right=-1
  for(let y=190;y<=355;y++)for(let x=0;x<512;x++)if(p.data[(y*p.width+texture.offset.x+x)*4+3]>230){left=Math.min(left,x);right=Math.max(right,x)}
  assert.ok(right>left,'tabletop must be present in actual PNG')
  return {center:((left+right)/2-f.anchor[0]*512)*f.scale[0]+f.x,width:(right-left+1)*f.scale[0],ground:(498-texture.offset.y-f.anchor[1]*texture.rectHeight)*f.scale[1]+f.y}
 })
