...
function Review(){
 ...
 <svg viewBox="0 0 384 576" style={{position:'absolute',inset:0,width:'100%',height:'100%',pointerEvents:'none'}}>{room==='street'&&<OldStreetBuildingEdges image={art.streetEdges}/>}<OldStreetFloor room={room} pixelShop compositeShop/>{walls&&<OldStreetRoomWalls room={room} facts={facts} compositeShop/>}<OldStreetDoorways room={room} facts={facts} woodImage={art.doorWood} stoneImage={art.stoneStair}/></svg>
 <div id="rpg" style={{position:'absolute',width:384,height:576,transformOrigin:'top left',pointerEvents:'none',zIndex:1}}/>
 {walls&&<svg viewBox="0 0 384 576" style={{position:'absolute',inset:0,width:'100%',height:'100%',pointerEvents:'none',zIndex:2}}><OldStreetRoomForeground room={room} facts={facts} actor={pos}/><OldStreetEntranceEaves room={room} image={art.streetEdges}/></svg>}
 </div></main>
}
