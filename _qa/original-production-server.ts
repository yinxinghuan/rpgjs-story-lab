...
const env={CARRIAGE_JOURNEYS:{idFromName:(id:string)=>id,get:(key:unknown)=>({fetch:(request:Request)=>{const id=String(key);let object=objects.get(id);if(!object){object=new worker.CarriageJourneyAuthority(storage.context(id),env);objects.set(id,object)}return object.fetch(request)}})}}
