...
export const photoCampaignFixture:OldStreetCampaignGenerator=async (context,signal)=>{
 if(process.env.OLDSTREET_QA_CAMPAIGN_REPORT)return campaignFixture(context,signal)
 ...
 const planned=context.route?investigationRoutePlan(context.route):undefined
 const ledgerSite=planned?.ledgerSite??process.env.OLDSTREET_QA_LEDGER_SITE
 if(ledgerSite&&!['archive','photo','laundry'].includes(ledgerSite))throw Error('QA_LEDGER_SITE_INVALID')
 return {...(ledgerSite&&ledgerSite!=='archive'?{ledgerSite}:{}),...(planned?.denseSource?{denseSource:planned.denseSource}:{}),title:sample.source.title,layout:'east-index',cards:sample.source.events.map((label:string,i:number)=>({id:['a','b','c','d'][i],label})),sources:{index:[{before:'a',after:'b'},{before:'c',after:'d'}],ledger:[{before:'b',after:'c'}]},discovery:sample.source.account}
}
