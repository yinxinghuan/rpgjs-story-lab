...
for(const locale of ['zh','en'] as const)test(`new ${locale} commission is visible before input; legacy opening is never retroactively rewritten`,()=>{
 ...
 const v4=runtime.initial(locale,randomUUID(),{campaign:'letter-trail-v4'})
 assert.equal(v4.campaign?.photoSource,'roof-negative-v1')
 assert.equal(v4.save.facts['roof-index-origin'],'archive')
 assert.equal(v4.save.facts['roof-index-read'],false)
 assert.ok(!JSON.stringify(campaignInputKnowledge(v4)).includes('north roof cabinet'),'the future film location is not known at enrollment')
 const v3=runtime.initial(locale,randomUUID(),{campaign:'letter-trail-v3'})
 assert.equal(v3.campaign?.version,3)
 assert.equal(runtime.upgrade(v3).campaign?.photoSource,undefined,'resuming a print-first journey never adds a film hunt')
