...
test('footsteps track displacement at 30/60/120 fps, not frame count',()=>{
 ...
 assert.ok(Math.max(...counts)-Math.min(...counts)<=1);assert.ok(counts.every(n=>n>=15&&n<=16))
})
...
test('material matches the room and low frame rate never creates a burst',()=>{
 for(const [room,cue] of [['shop','wood'],['street','stone']] as const){const s=new StreetFootsteps();s.update(.2,{x:0,y:0},room,false);assert.equal(s.update(.2,{x:20,y:0},room,false),cue)}
})
