...
test('immutable recipe uses corrected B gait; equipment never inherits actor identity; namespace survives Remix',()=>{
 for(const r of SPRITE_RECIPES){const d=planSpriteGeneration(r,GAME_ID),request=spriteGenerationRequest(r,d.id,GAME_ID);assert.equal(request.mode,'edit');assert.equal(request.referenceUrls!.length,1);assert.ok(request.prompt.length<2400);assert.equal(request.size.width%64,0);assert.equal(request.size.height%64,0);assert.deepEqual(request,spriteGenerationRequest(r,d.id,GAME_ID));if(r==='ada-walk-v1'){assert.match(request.referenceUrls![0],/hero-gait-v2/);assert.match(request.prompt,/opposite walking strides/)}else assert.doesNotMatch(request.prompt,/Ada|female|apprentice/)}
...
test('ambiguous lost submission keeps request ID and cooldown; successful response is retained without changing active source',async()=>{
 ...
 const d=planSpriteGeneration('ada-walk-v1',GAME_ID);await r.put(d);const ids:string[]=[];let first=true
...
test('task ID persists before download; reopen resumes GET only and verifies PNG hash plus native decode',async()=>{
 const f=new IDBFactory();let r=store('resume',f);const d=planSpriteGeneration('ada-walk-v1',GAME_ID);await r.put(d)
...
test('rate limiting honors delay, terminal origin rejection cannot be resumed, history keeps earlier results',async()=>{
 const r=store();const d=planSpriteGeneration('ada-walk-v1',GAME_ID);await r.put(d);let calls=0
...
test('wrong request/task/media, untrusted URL, wrong PNG dimensions and failed native decoding never become ready',async()=>{
 for(const kind of ['request','task','format','url','size','bytes','decode']){
  const r=store(kind),d=planSpriteGeneration('ada-walk-v1',GAME_ID);if(kind==='task')d.taskId='expected-task';await r.put(d)
...
test('late generation completion cannot replace a newer request or source',async()=>{
 const r=store();await r.put(planSpriteGeneration('ada-walk-v1',GAME_ID));let newer:any
