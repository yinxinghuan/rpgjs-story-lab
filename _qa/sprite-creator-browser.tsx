...
async function fixture(){const width=72,height=96,rgba=new Uint8ClampedArray(width*height*4);for(let y=0;y<height;y++)for(let x=0;x<width;x++){const sx=x%24,sy=y%24,v=(Math.floor(x/3)+Math.floor(y/3))%2?225:250;rgba.set(sx>=7&&sx<17&&sy>=4&&sy<20?[25,80,100,255]:[v,v,v,255],(y*width+x)*4)}return encodeSpritePixels({width,height,rgba})}
