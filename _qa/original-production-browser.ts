...
try{for(const [width,height,lang,travel]of [[320,568,'zh',true],[390,844,'en',false]]as const){
 ...
 try{
  await page.goto(origin+(heroBinding?'/?art=baseline':'/'));await ready(0);await close();const journey=head.id,bindings=structuredClone(head.assets);if(heroBinding){assert.equal(originalHeroVersion(bindings),ORIGINAL_HERO_V2);assert.ok(heroLoads.includes(originalHeroRelease().resource.sha256))}await inspect('opening')
  ...
  for(const [i,a]of steps.entries()){
   ...
   if(['pine-invite','pine-stay','yard-invite','yard-stay','pass-debrief','bridge-arrange','junction-review'].includes(a)){
    ...
    await page.waitForTimeout(2300);await page.screenshot({path:`_qa/ui/${prefix}-${a}-${width}-${lang}-local-qa.png`});await page.reload();await ready(i+1);await page.waitForTimeout(500);await inspect(a+'-restored');assert.deepEqual(head.assets,bindings)
   }
