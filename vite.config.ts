import {oldStreetWorkerPreviewPlugin} from './server/old-street-worker-preview'
import {oldStreetDevPlugin} from './server/old-street-dev-plugin'
import { defineConfig } from 'vite'
import canvasengine from '@canvasengine/compiler'
import { labPlugin } from './server/service'
import {preflightPlugin} from './server/preflight-plugin'
import {sceneResourceManifest} from './server/scene-resource-manifest'
import {originalScenePreviewPlugin,originalScenePreviewDefinition,originalStoryPreviewDefinition} from './server/original-scene-preview'
export default defineConfig(({mode})=>({base:'./',build:{rollupOptions:{input:{index:'index.html',creator:'creator.html'}}},define:{__ORIGINAL_STORY_PREVIEW__:JSON.stringify(['cloud','pages','cloud-preflight'].includes(mode)?originalStoryPreviewDefinition():null),__ORIGINAL_SCENE_PREVIEW__:JSON.stringify(originalScenePreviewDefinition()),__SCENE_RESOURCES__:JSON.stringify(sceneResourceManifest()),__QA_PERFORMANCE__:JSON.stringify(mode==='pages'&&process.env.CARRIAGE_QA_PERF==='1')},plugins:[canvasengine(),originalScenePreviewPlugin(['cloud','pages','cloud-preflight'].includes(mode)),...(mode==='oldstreet-dev'?[oldStreetDevPlugin(),oldStreetWorkerPreviewPlugin()]:mode==='cloud-preflight'?[preflightPlugin(),oldStreetWorkerPreviewPlugin()]:['pages','cloud'].includes(mode)?[]:[labPlugin()])],resolve:{dedupe:['@signe/reactive','@signe/di','canvasengine','pixi.js','@rpgjs/common']},optimizeDeps:{include:['pixi.js > @xmldom/xmldom']}}))
