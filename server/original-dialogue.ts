...
/** Explicitly unobservable art details cannot become facts via a second model's
 * approval. This narrow authored answer follows the bound artwork, not history.
 * Other visual questions still use the normal generation/review pipeline. */
export function originalVisualUncertaintyReply(input:string,c:OriginalDialogueContext):string|null{
 const speaker=c.visuals.speaker
 if(!('unestablishedDetails' in speaker)||!speaker.unestablishedDetails?.includes('lamp-fastening'))return null
 if(!/(?:小灯|你的灯|灯具|lamp)/i.test(input)||!/(?:固定|绑|系|带子|挂|扣|左|右|fasten|attach|strap|tied|tie\b|secur|hang|hung|clip|left|right)/i.test(input))return null
 return c.locale==='zh'?'你能看见小黄铜灯的上部露在我腰间工具袋的袋口上方。从这个角度能看到的部分，无法判断它具体怎么固定。':'You can see the upper part of my small brass lamp above the opening of the tool pouch at my hip. This view does not show how it is fastened.'
}
function unsupportedVisualBodySide(text:string,c:OriginalDialogueContext){
 const speaker=c.visuals.speaker
 // "screen-right" is a camera coordinate, never evidence of anatomical side.
 // Fail closed on these concrete body-relative assertions; the author answer
 // above handles explicit questions without forcing an ambiguous model answer.
 return 'unestablishedDetails' in speaker&&speaker.unestablishedDetails?.includes('lamp-body-side')&&
  /(?:my|your)\s+(?:own\s+)?(?:left|right)\s+(?:hand|side|hip)\b|我(?:的|自己(?:的)?)?[左右](?:侧|边|手|腰)|你(?:的|自己(?:的)?)?[左右](?:侧|边|手|腰)/i.test(text)
}
...
export function createOriginalDialogueGenerator(request:ModelRequest,budgetMs=20000):OriginalDialogueGenerator{
 ...
 return async(input,context)=>{
  ...
  const visualUncertainty=originalVisualUncertaintyReply(input,context);if(visualUncertainty!==null)return visualUncertainty
