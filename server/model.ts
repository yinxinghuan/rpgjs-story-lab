 ...
 for(let attempt=0;attempt<2;attempt++){
  try {
   ...
   const candidate:Proposal=raw?.kind==='action'
    ? {kind:'action',actionId:raw.actionId,...(raw.content!==undefined?{content:raw.content}:{}),entityIds:[target],claims:[],text:''}
    : raw
