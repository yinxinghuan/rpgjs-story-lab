...
export async function handleOriginalSession(request:Request,owner:string,authority:OriginalTrainAuthority,readBody:(request:Request)=>Promise<any>,resolveBackground?:(id:string)=>Promise<PublishedBackground>,resolveDevice?:(id:string)=>Promise<PublishedDevice>,resolveActor?:(id:string)=>Promise<PublishedActor>){
 try{
  ...
  if(path==='/sessions'&&request.method==='POST'){
   ...
   if(Object.keys(b).some(k=>!['enrollment_id','locale','backgroundRelease','deviceRelease','actorRelease'].includes(k))||!['zh','en'].includes(b.locale))throw new LabError('INVALID_ENROLLMENT')
   ...
   return originalJson(authority.create(owner,b.enrollment_id,b.locale,device||actor?{...(device?{starter:device}:{}),...(actor?{actor}:{}),...(release?{background:release}:{})}:release))
  }
