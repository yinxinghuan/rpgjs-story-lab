...
import {MAP_VERSION} from '../src/contract'
...
export async function exportJourney(db:AuthorityStorage,owner:string,id:string):Promise<BackupEnvelope>{
 const payload=db.transaction(()=>{
  ...
  return {format:'carriage-journey-backup-v1' as const,gameId:GAME_ID,mapVersion:MAP_VERSION,createdAt:new Date().toISOString(),journey,journal:db.all<JournalRow>('SELECT * FROM journal WHERE session=? ORDER BY cursor',id),receipts:db.all<ReceiptRow>('SELECT r.* FROM receipts r JOIN journal j ON j.action=r.action WHERE r.owner=? AND j.session=? ORDER BY j.cursor',owner,id)}
 })
...
export async function validateBackup(value:unknown):Promise<JourneyBackup>{
 ...
 requireValue(p?.format==='carriage-journey-backup-v1'&&p.gameId===GAME_ID&&p.mapVersion===MAP_VERSION)
 ...
 requireValue(head.id===j.id&&head.mapVersion===MAP_VERSION&&Number.isSafeInteger(j.cursor)&&j.cursor>=0&&head.version===j.cursor&&Number.isFinite(j.updated))
