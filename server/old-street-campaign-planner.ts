...
import {compilePreparedInvestigation} from './old-street-investigation-draft'
...
export function createOldStreetCampaignPlanner(request:ModelRequest):OldStreetCampaignGenerator{
 return async(context,signal)=>{
  ...
  const contract=context.stage==='archive'
   ...
   : 'Return {title,recordAt:"start"|"end",events:[string,string,string,string],roomPlan:{indexSide:"left"|"right",storageShelves:0|1|2|3,rack:"none"|"left"|"right"}}. Create ONE complete coherent historical episode, not separate unconnected maintenance facts. previous.label is one already fixed historical event; put it at the start or end using recordAt. events contains exactly FOUR short events in chronological order, including previous.label VERBATIM as the first event when recordAt=start or last event when recordAt=end. Together they form one plausible account; complaints/planning/acquisition must precede the same repair/installation, unless a subsequent defect is explicitly part of this account. Do not put a completed repair before its own motivation and preparations. Each event max70 characters; the event at the opposite endpoint max50. Event labels are standalone past-tense actions, without before/after/following/prior to or relative timing clauses; the game will provide relation evidence separately. Prefer endpoints whose order is worth checking in records, while keeping the history believable. Do not write a fragment or opening synopsis. The engine uses the two actual endpoint entries to make an undated opening clue and question without revealing chronology. title max60, preferably 2-5 words. roomPlan is spatial intent only; do not draw a grid. The complete sequence is prepared once; players later discover parts at two shelves, arrange event cards, and choose whether to preserve the findings. Do not write new rewards, player actions or extra fields.'
  ...
  for(let attempt=0;attempt<2;attempt++){
   ...
   const task=context.stage==='parcel'&&context.investigation?{
