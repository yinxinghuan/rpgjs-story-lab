import {sharedEvidenceKnowledge} from '../src/old-street-shared-evidence'
import {publicRecordKnowledge} from '../src/old-street-public-record'
import {oldStreetKeyStatus} from '../src/old-street-letter-guidance'
...
import {oldStreetAttemptHistory,oldStreetConversation,oldStreetTalkTopics} from '../src/old-street-conversation'
export function oldStreetDialogueContext(h:OldStreetHead,entity:string){
 ...
 return {locale:h.save.locale,sceneId:h.sceneId,speaker:{id:p.id,name:person.name},
  knowledge:[{id:'current-place',text:h.save.locale==='zh'?`玩家和你目前都在${oldStreetRooms[h.sceneId as OldStreetRoom][0]}。其他提及的地点不是当前所在地。`:`You and the player are currently at ${oldStreetRooms[h.sceneId as OldStreetRoom][1]}. Other places mentioned are not the current location.`},...(entity==='watchmaker'?[{id:'key-status',text:oldStreetKeyStatus(h.save)},{id:'key-use',text:h.save.locale==='zh'?'小格钥匙只用于打开修表铺的寄信小格。用完可在河边工作棚交还给修表师；借钥匙不要求玩家送钟。':'The compartment key opens the letter compartment in the watch shop. After use, return it to the watchmaker at the riverside workshop. Borrowing it does not require delivering the clock.'}]:[]),{id:'optional-help',text:h.save.locale==='zh'?'取信是主要目的。送钟、找照片及记录旧事都是可选帮助，不能把它们说成取信、借钥匙或回家的前置条件。':'Collecting the letter is the main purpose. Returning the clock, finding photographs and recording memories are optional help, never prerequisites for collecting the letter, borrowing the key or going home.'},...oldStreetTalkTopics(h.save,entity).map(t=>({id:t.id,text:t.reply})),...oldStreetSceneKnowledge(h.save,h.sceneId),...publicRecordKnowledge(h),...sharedEvidenceKnowledge(h.save,entity)],
  recentTurns:oldStreetConversation(h.save,p.id),
  recentAttempts:oldStreetAttemptHistory(h.save,entity),
