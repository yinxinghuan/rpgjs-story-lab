...
import {OLD_STREET_API_PATH,OLD_STREET_RUNTIME_HEADER,OLD_STREET_RUNTIME_CONTRACT,OLD_STREET_RELEASED,OLD_STREET_NARRATION_PREVIEW,OLD_STREET_EXPANSION_RELEASED,OLD_STREET_PREVIEW_RELEASED,OLD_STREET_PREVIEW_VERSION} from '../src/old-street-runtime-contract'
...
import {assertPublishedActor,assertPublishedHero,actorReleaseId} from '../src/actor-publication'
...
const oldStreetPreviewPresentation:OldStreetGate=()=>true
...
export class CarriageJourneyAuthority{
 ...
 constructor(ctx:DurableContext,private env?:Environment,modelRequest?:ModelRequest,imageProducer?:ImageProducer,originalGate:OriginalPresentationGate=ORIGINAL_STORY_RELEASED?originalReleasedPresentation:originalPresentationUnavailable,private originalInterpreter?:OriginalActionInterpreter,private originalDialogue?:OriginalDialogueGenerator,private artSource?:ArtArchiveSource,private illustrationProducer?:IllustrationProducer,private oldStreetGate:OldStreetGate|undefined=OLD_STREET_PREVIEW_RELEASED?oldStreetPreviewPresentation:undefined,private oldStreetInterpreter?:OriginalActionInterpreter,private oldStreetDialogue?:OldStreetDialogueGenerator,private expansionProviders?:{model:ModelRequest;photo:ExpansionPhotoProducer}){
  ...
  if(oldStreetGate===oldStreetPreviewPresentation&&OLD_STREET_NARRATION_PREVIEW){
   this.oldStreetInterpreter??=createOriginalActionInterpreter(modelRequest??chatModel)
   this.oldStreetDialogue??=createOldStreetDialogueGenerator(modelRequest??chatModel)
  }
 ...
 async fetch(request:Request){try{
  ...
  if(url.pathname.startsWith(CREATOR_API_PATH+'/')){
   ...
   try{
    ...
    const heroPublished=path.match(/^\/hero-releases\/([a-f0-9]{64})\.([a-f0-9-]{36})(\/file)?$/)
    if(heroPublished&&request.method==='GET'){
     if(heroPublished[1]!==owner)throw new LabError('HERO_NOT_PUBLISHED',404)
     this.sprites??=new CreatorSpriteArchive(this.db)
     const release=this.sprites.heroPublication(owner,heroPublished[2]);if(!release)throw new LabError('HERO_NOT_PUBLISHED',404)
     if(!heroPublished[3])return respond(release)
     return new Response(new Uint8Array(await this.sprites.file(owner,heroPublished[2],'candidate')),{headers:{'Content-Type':'image/png','Cache-Control':'public, max-age=31536000, immutable',[RUNTIME_HEADER]:RUNTIME_CONTRACT,[CREATOR_RUNTIME_HEADER]:CREATOR_RUNTIME_CONTRACT}})
    }
  ...
  if(url.pathname.startsWith(OLD_STREET_API_PATH+'/')){
   this.oldstreet??=new OldStreetAuthority(this.db,this.oldStreetGate,this.oldStreetInterpreter,this.oldStreetDialogue,h=>this.expansions?.candidateFor(h),h=>this.expansionMedia?.candidateFor(h))
  ...
  if(url.pathname.startsWith(ORIGINAL_API_PATH+'/')){
   ...
   },this.illustrations&&this.illustrationProducer?{store:this.illustrations,produce:this.illustrationProducer}:undefined,async id=>{
    if(!actorReleaseId(id)||!this.env?.CARRIAGE_JOURNEYS)throw new LabError('HERO_NOT_PUBLISHED',404)
    const publisher=id.split('.')[0],r=await this.env.CARRIAGE_JOURNEYS.get(this.env.CARRIAGE_JOURNEYS.idFromName('creator-art-v1:'+publisher)).fetch(new Request('https://authority.invalid/api/creator/hero-releases/'+id,{headers:{'X-Authority-Owner':publisher,[RUNTIME_HEADER]:RUNTIME_CONTRACT,[CREATOR_RUNTIME_HEADER]:CREATOR_RUNTIME_CONTRACT}}))
    if(!r.ok)throw new LabError('HERO_NOT_PUBLISHED',404)
    const release=await r.json();assertPublishedHero(release);if(release.id!==id)throw new LabError('ACTOR_RELEASE_INVALID',409);return release
   })
  }
